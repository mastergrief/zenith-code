...
- **PT+Delta / `CopyAugmentedDeltaNet`** (R-delta-6a/20, commits `31337f3`/`63a49fc`, file `calm/llm_computer/copy_augmented_delta.py`) — new default trained-card architecture. Copy gate + DeltaNet Householder fast-weight backbone. Matches plain PT on structure (99.5% NL math) + wins +66-84pp on retrieval tasks (MQAR N=10-20). Chunkwise UT-transform training (3-7× speedup) + cached `decode_greedy_cached` (1.18× plain-PT inference). Plain PT stays as ablation baseline. Full spec: `.claude/rules/delta_rule.md`.
