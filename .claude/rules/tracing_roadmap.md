...
null, fused flash-attn correctness=1.0 but short-ctx perf regression,
tq4 matvec v2 shipped (-7%), v3/v4 null, BLOCK_M sweep null, MAX_TOKENS
starvation receipt (R53.25), sandbox stdlib pre-import fix (R53.22),
substrate L41 install regression on code (R53.14/20a/20b).
...
| KVCacheTq4 dequant-on-read at decode (pre-R53.34) | Round 53.28 → 53.33 | Initial `KVCacheTq4.update()` dequanted the FULL cached sequence every call → O(N) dequant per step, O(N²) across full decode. Measured cost: linked_list 5/5 took 806s with tq4 KV vs 94s with fp16 KVCache (~8× slowdown). R53.33 diagnosed; reverted `USE_TQ4_KV=False` in eval scripts until R53.34 fused path landed. Later: Phase 1 memoized dequant path (not the fused attention kernel) is the actual shipped win — ~77% of fp16 tok/s at ~50% KV memory. See `turboquant.md` §"Fused flash-attention decode (R53.34)". |
| Fused tq4 flash-attn at short context (N≤1024) | Round 53.34 | Correctness=1.0 cosine vs fp32 ref at all tested N; real-Gemma ablation Δmean=0, argmax=+0. Perf: 8-10% SLOWER than Phase 1 memoized dequant at N≤1024 (N=64 5.60 vs 6.06 tok/s; N=1024 5.11 vs 5.60). Root cause: 8 × 42 = 336 per-Q-head kernel launches per decode step; launch overhead dominates streaming-byte-load advantage at measured N. Asymptotic crossover (N≫2K) NOT measured (20+ min/run budget). Shipped `_use_fused_flash_attn=False` default. Kernel kept in tree. To unlock perf at short ctx: (a) 1 Triton kernel spanning all Q heads (→ 1 launch/layer), (b) parallel-over-N V kernel via TILE_N. Both non-trivial. |
...
| Fused tq4 flash-attn decode kernel (Phase 2 of TurboQuant KV work) | Round 53.34 | Rewrote the 257-line `tq4_flash_attn.py` scaffold with correct online-softmax + per-head-parallel V kernel, wired into `_forward_layer` for `KVCacheTq4 AND S==1 AND d_head==256 AND not partitions`. **Correctness validated**: 7/7 unit tests pass with cosine=1.00000 vs fp32 reference at N∈{16,64,128,256,1024}; real-Gemma ablation shows Δmean=0.0000 argmax=+0 vs dequant path. **Perf regression at measured N**: clean A/B (Triton weight kernels ON for both, isolated via `enable_fused_flash_attn()` flag) shows fused is 8-10% SLOWER than Phase 1 memo path — N=64 5.60 vs 6.06 tok/s, N=256 5.32 vs 5.77, N=1024 5.11 vs 5.60 (fp16 baseline 7.00-7.48). **Root cause**: per-Q-head Python loop in `fused_tq4_flash_attn_decode` launches 8 × 42 = 336 small `tq4_matvec_triton` kernels per decode step; launch overhead dominates over the streaming-byte-load advantage. At N≤1024 the Phase 1 memoized dequant path does a single wide Triton tq4 dequant + cuBLAS einsum which wins on the same RTX 4070M. **Asymptotic crossover hypothesis** (N≫2K where KV bandwidth dominates) NOT tested — generation at N=4K+ hits practical time budget limits (20+ min/run). **Default disposition**: `_use_fused_flash_attn=False` shipped default, kernel kept in tree for future long-context experiments. Phase 1 memo alone gets us to ~77% of fp16 tok/s at 50% KV memory — that's the real shipped win. To recover Phase 2 perf at short context would need: (a) one Triton kernel spanning all Q heads (remove Python loop — 1 launch/layer instead of 8), (b) parallel-over-N V kernel via TILE_N blocking (instead of current serial N-loop per program). Both are non-trivial; reconsider if N>4K is the target workload. |
