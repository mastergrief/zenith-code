...
**llama.cpp (primary)** — base model via full GPU:

- **Production GGUF**: `~/models/<your-model>-tq4-aligned.gguf` (~5.0 GB, TurboQuant tq4, 132-byte block alignment for 4-byte aligned CUDA loads). **This is what CALM runs on.**
- **Alternative GGUFs**: stock and fine-tuned `Q5_K_M` builds in `~/models/` (~2.9-5.5 GB)
...
- Launch (substitute your GGUF): `llama-server -m ~/models/<your-model>.gguf --ctx-size 524288 --parallel 1 --cache-type-k tq4_k256 --cache-type-v tq4_k256 -ngl 999 --port 8080`
...
- Hot-swap: `agents/model_swap.py:LlamaServerManager`. `/swap <substring>` in harness.

**Ollama (fallback)** — stock models, quick testing:

- Pulled models **include** (enumerate via `curl -s localhost:11434/api/tags`): stock base tags plus custom Modelfile builds, e.g. `reasoning-base:latest`
- Custom Modelfiles in `models/` **include** fast variants at 2048 and 8192 ctx, and `Modelfile.reasoning-base` (32K ctx)
...
- **llama.cpp**: built at `~/llama.cpp/build/bin/` with CUDA support (RTX 4070). **Branch `zenith`** carries 3 custom commits beyond upstream (see `git log` for SHAs):
  ...
  - **gate+up ordering fix**: upstream fusion check at `should_fuse_mul_mat` rejected reversed gate/up ordering — GLU fusion was never firing on any quant type for such models upstream. Worth upstreaming.
  ...
  - **Local patch** at `tools/server/server-context.cpp:763-766` — comments out `n_ctx_slot = n_ctx_train` for >128K context. Re-apply after `git pull`.
...
- **8 GB VRAM**: production default is tq4 weights + tq4 KV at **512K context** (~7 GB total, see Serving Architecture above). Historical Q4-KV + Q5_K_M configs fit at **256K context**. 9B Q4 fits at 2K (Ollama). 0.8B FP16 fits at 32K.
- **Both 4B bases are trained at 256K context** (verified via GGUF metadata `n_ctx_train=262144`).
- **4B QLoRA**: 248K vocab CE loss OOMs on anything under 40GB VRAM. Must use cloud GPU (Colab A100).
- **0.8B QLoRA**: fits locally at batch=1, seq_len=1024, packing=false
