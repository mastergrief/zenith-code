...
Full spec: `.claude/spec/Substrate.md`
...
- **DT (Delta-Transducer) / `CopyAugmentedDeltaNet`** (R-delta-6a/20, commits `31337f3`/`63a49fc`, file `calm/llm_computer/copy_augmented_delta.py`; renamed PT+Delta → DT 2026-04-22) — default trained-card architecture **for retrieval + structure-extraction regimes**. Copy gate + DeltaNet Householder fast-weight backbone. Matches plain PT on structure (99.5% NL math) + wins +66-84pp on retrieval tasks (MQAR N=10-20). Chunkwise UT-transform training (3-7× speedup) + cached `decode_greedy_cached` (1.18× plain-PT inference). Plain PT stays as ablation baseline. Full spec: `.claude/rules/delta_rule.md`. **Code-skeleton DT** (NL → `def FN(<args>):`) is an open arc at 0.193 honest val ep16 (520 held-out, v13) — recipe is different: needs R26 aux copy-loss + R27 split-before-aug + gate init -1.0 + EMA decay 0.995. Don't extrapolate retrieval defaults. See `delta_rule.md` §DT code-skeleton arc.
...
- **Prod Gemma install (session 32, updated 2026-04-22 R22f)**: retrieval PTs install via `CardSlot(layer_idx, ch_off, pt, d_card=80, card_input_fn=adapter, use_full_residual=True, output_fn=writer).attach(m, preserve=False)` with aligned `write_margin=min_margin=14.5` (R22 4-gate default post-R22f recalibration — 60/60 on pooled corpus, see `delta_rule.md` §R22 install). `preserve=True` remains for chained cards where channel isolation is load-bearing; it subtly pins reserved channels even when card is silent (R22b r6 `q=v margin=0.00` regression), so is not the retrieval default. PT's copy-augmented attention can't reduce to a sub-head mode, so CardSlot (separate forward + additive residual write + optional preservation masking) is the right pattern. Chained CRLM proven inside one Gemma forward: `copy_augmented_hrm` PT writes structure log_probs at ch[2400:2480] → adder_tiny reads those channels via `card_input_fn`, computes the answer, writes at ch[2480:2488] → `VerificationHook` biases Gemma's BPE digit logit. See commit `f5455f6` (session 32 chain) + `73df738` (R22 shipped at 22.0) + `9691e06` (R22f 14.5 recalibration).
...
Full bench receipt + methodology in `.claude/spec/turboquant.md`.
