"""Round 32: per-head at L34 for induction. Is it concentrated or diffuse?

R31: L34 (SWA) is the induction peak — Δ=-1.169, 20/20 hurts, std 0.26.
Test: which of its 8 Q heads carry the signal?

Hypothesis: induction should be concentrated per-head (like arithmetic
at L23 H1+H4 = 93% of signal). If so, induction is compilable as a
specific attention LookUp circuit.

Also probe L33 and L37 as secondary clusters.
"""
...
def build_induction_prompts(n_prompts=20, seq_len=4, seed=0):
    random.seed(seed)
    ...
    for _ in range(n_prompts):
        ...
        prompts.append((" ".join(seq), chosen[2]))
    return prompts
...
def main():
    ...
    print("[induction-per-head] loading substrate...")
    ...
    prompts = build_induction_prompts(n_prompts=20, seq_len=4, seed=0)
    ...
    for prompt, expected in prompts:
        ...
        expected_tok = "▁" + expected
        if argmax_tok == expected_tok:
            base_logit = logits[0, -1, argmax].item()
            baselines.append((prompt, expected, argmax, token_ids, base_logit))

    print(f"  {len(baselines)}/{len(prompts)} clean baselines")
