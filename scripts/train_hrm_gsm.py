"""Train HRM on GSM-style word problems.

Usage:
  PYTHONPATH=. python3 scripts/train_hrm_gsm.py \\
    --epochs 500 --problems 2000 --hidden 32 --max-enc 128 --max-dec 28
"""
...
from calm.hrm.gsm_data import GSMDataGenerator, GSMDataset
from calm.hrm.model import HRMConfig, HRMSeq2Seq


CHECKPOINT_PATH = Path("calm/hrm/checkpoints/gsm_best.pt")


def train(epochs=500, problems=2000, batch_size=128, lr=1e-3, hidden=32,
          num_heads=4, l_layers=1, h_layers=1, dec_layers=1,
          max_enc=128, max_dec=28, seed=42):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[gsm] generating {problems} GSM-style problems...")
    gen = GSMDataGenerator(seed=seed)
    probs = gen.generate(problems)
    print(f"[gsm] first 3 examples:")
    for p in probs[:3]:
        short = p.problem if len(p.problem) < 95 else p.problem[:92] + "..."
        print(f"  [{len(p.problem):3}ch] {short:95} → {p.expression}")

    ds = GSMDataset(probs, max_enc_len=max_enc, max_dec_len=max_dec)
    ...
    print(f"[gsm] model: {model.param_count():,} params "
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 50 == 0 or epoch == 1:
            ...
            print(f"[gsm] epoch {epoch:4d}/{epochs}: loss={total/max(nb,1):.4f}, "
            ...
            if va > best:
                ...
                torch.save({
                    ...
                    "epoch": epoch, "val_acc": va, "gsm": True,
                }, CHECKPOINT_PATH)
    print(f"[gsm] done: {time.time()-t0:.0f}s, best val_acc={best:.1%}")
