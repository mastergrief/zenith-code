"""Round 37: per-head at L35 (comparison peak) and L23 (shared with
arithmetic) — find the comparison specialist head.

R36: comparison sweep showed L35 (GLB, d_head=512) is the dominant
layer — Δ=-1.771, 18/18 hurts. L23 (GLB) is secondary at -1.504,
11/18. Question: concentrated specialist head (Category A, like
arithmetic L23 H1/H4) or diffuse (Category C)?

Concentrated → Tier-2-compilable comparison circuit.
Diffuse → FFN-locked, needs ROME/MEMIT.
"""
...
def main():
    ...
    print("[comparison-per-head] loading substrate...")
    ...
    for L in TARGET_LAYERS:
        ...
        lyr = "GLB" if d_head > 256 else "SWA"
        print(f"  L{L}: d_head={d_head} ({lyr})")
