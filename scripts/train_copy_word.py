"""Train copy-augmented HRM on word problems.

Same architecture as train_copy_augmented_hrm.py but uses
WordProblemGenerator. Tests the pointer-copy mechanism on longer
NL inputs with names, pronouns, multi-step actions.
"""
...
CHECKPOINT_PATH = Path("calm/hrm/checkpoints/copy_word_best.pt")


class WordSeqDataset(Dataset):
    """Pack (word problem, expression) into token sequence with loss mask."""
    ...
    def __getitem__(self, idx):
        ...
        # WordProblem uses .problem not .question
...
def _autoreg_eval(model, problems, device, max_gen=30):
    """Autoregressive eval — greedy decode, exact expression match."""
    bos = _CHAR_TO_ID["<bos>"]
    sep = _CHAR_TO_ID["<sep>"]
    eos = _CHAR_TO_ID["<eos>"]
    model.eval()
...
def train(epochs=500, problems=5000, batch_size=64, lr=1e-3,
    ...
    print(f"[copy-word] generated {len(probs)} word problems")
    ds = WordSeqDataset(probs, max_len=max_len)
    ...
    print(f"[copy-word] model: {total_params:,} params on {device}")
    if scheduled_sampling:
        print(f"[copy-word] scheduled sampling: tf_ratio {tf_ratio_start:.1f}"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            print(f"[copy-word] epoch {epoch:4d}/{epochs}: "
    ...
    print(f"[copy-word] done: {time.time()-t0:.0f}s, best autoreg={best_autoreg:.1%}")
