#!/usr/bin/env python3
"""Apply dense-[0..9] n=32 structural-vs-sampling decider packet (forked from n32 CA family)."""
...
DRAFT = (
    REPO / "artifacts/consensus_prep/c4s1_n32_dense_09_ca_decider_launch_packet_v1_draft.json"
)
REPLAY = (
    REPO
    / "artifacts/consensus_prep/c4s1_n32_dense_09_ca_decider_launch_packet_v1_replay_commands.json"
)
HEAD = "58568e272df6faa53bf3bc001c09d683fd270669"
ACTIVE_TASK_ID = "1782633464140-b85ec12a"
UPSTREAM_TASK_ID = "1782633464140-b85ec12a"
RUN_ROOT = "/home/gabe/hrm158_c4s1_phase3_gpu_gate/C4S1_PHASE3_N32_DENSE_09_CA_DECIDER_V1"
RUN_ID = "C4S1_PHASE3_N32_DENSE_09_CA_DECIDER_V1"
N_STATES = 32
PACKET_REVISION = "v1_n32_dense_09_ca_decider_58568e2"
FOLD1_SPEC = (
    "artifacts/measurement_closeout/c4s1d7_n32_ca_confirmation_informative_null_and_sampling_fork_spec.md"
)
...
PINS: dict[str, str] = {
    ...
    "scripts/hrm_text_158_code_currency_guard.py": (
        "ea1a39678f988182cb23b85a108ca8f00d281bb2e0a0504cc6a71c4634044fc3"
    ),
    ...
    "calm/hrm_text_158/native_full_stack/host_tracemalloc_probe.py": (
        "7ce168c3e06e0378fbf67a7bb1f007b6142313ad8f3805c71cd9f143cbcb669d"
    ),
    ...
    "calm/hrm_text_158/native_full_stack/bounded_delta_learner.py": (
        "f34bb3a11c619e4ddf7c48e69e0e18a8f77efc8def6dc95848e57106f0b3a797"
    ),
}
...
def _update_pin_blocks(draft: dict[str, Any]) -> None:
    ...
    draft["code_pins_note"] = (
        "test_slice5 omitted (not launch-executed). git_head_required=58568e2 is the "
        "FIX-C-PROPER science baseline (FDC); descendant HEAD allowed via merge-base "
...
def build_proof_artifacts() -> dict[str, str]:
    ...
    return {
        ...
        "ca_confirmation_wrapper_receipt": f"{RUN_ROOT}/prelaunch/ca_confirmation_wrapper_receipt.json",
        ...
        "resource_lane_holding": f"{RUN_ROOT}/prelaunch/resource_lane_holding.json",
        "resource_lane_release_witness": (
            f"{RUN_ROOT}/post_gpu/resource_lane_release_witness.json"
        ),
        ...
        "primary_per_state": (
            f"{confirmation_root}/callsite_band_counter_ca_confirmation_receipt.json#per_state"
        ),
        "fallback_trigger": f"{RUN_ROOT}/prelaunch/ca_confirmation_wrapper_receipt.json#fallback_trigger",
        "primary_dense_fork_readable": (
...
STALE_RENDER_PATTERNS: tuple[tuple[str, str], ...] = (
    ...
    ("C4S1_PHASE3_N32_CA_CONFIRMATION_V1", "stale_n32_4point_run_id"),
)
...
def build_draft() -> dict[str, Any]:
    ...
    draft["schema_version"] = "hrm_text_158_c4s1_n32_dense_09_ca_decider_launch_packet/v1"
    draft["run_id"] = RUN_ID
    ...
    draft["design_binding"] = {
        ...
        "chosen_option": "n32_dense_09_structural_vs_sampling_decider",
    }
    ...
    draft["decision_contract"] = {
        "chosen_path": (
            f"dense-[0..9] decider via run_callsite_band_counter_ca_confirmation @ {HEAD[:7]} "
            "n_states=32; HRM_TEXT_158_OBMALLOC_EXPANDED_SAMPLED_STATES=0..9 shell export; "
    ...
    draft["ca_confirmation_objective"] = {
        "science_question": (
            "Structural vs sampling fork: does any state in [1..9] have S1d.7 crossings at "
            "n=32, or is state-0-only crossing structural?"
        ),
        ...
        "anti_overclaim": (
            "dense-[0..9] decider PRECONDITION only; NO CA verdict; NO candidate-C; NO "
            "reduction eligibility; NOT sub-2 proof; NOT 430MB bank pin"
        ),
    }
    ...
    proof["description"] = (
        f"dense-[0..9] n={N_STATES} structural-vs-sampling decider via "
        ...
        "dense fallback; primary_dense_fork_readable salvage on completed primary"
    )
    proof["interpretation_rule"] = (
        "Fork read: evaluate primary_dense_fork_readable FIRST. When true, read "
        "structural-vs-sampling from runs.primary.per_state EVEN IF "
        "science_verdict_source==fallback and fallback_trigger.rss_breach==true. "
        "When false, primary is partial telemetry only; dense n=16 fallback is science "
        "source. Partial JSONL rows, nonzero runs.B exit, subprocess_timeout_expired, or "
        "failed mark coverage CANNOT decide the fork (fail-closed). "
        "terminal_branch (NOT receipt.ok/infra_ok) for CA-share branches only. "
        "NO CA verdict / candidate-C / reduction eligibility. "
        "Launch wrapper receipt MUST surface runs.primary.per_state, primary_receipt_path, "
        "fallback_trigger, primary_dense_fork_readable."
    )
    ...
    proof["pass_criteria"] = {
        "dense_env_export": (
            ...
            f"{DENSE_SAMPLED_STATES_VALUE} AND env_toggles_b_arm matches"
        ),
        "mark_count_eq_sampled_states": (
            f"s1d7_band_counter_mark_count==len(sampled_states)=={EXPECTED_MARK_COUNT} "
            f"(dense [0..9] at n={N_STATES}; same dense env on n={FALLBACK_N_STATES} fallback)"
        ),
        "primary_dense_fork_readable": (
        ...
        "abort_fail_closed": (
            ...
            "CANNOT decide fork; launch acceptance fail-closed on abnormal exit"
        ),
        ...
        "feasibility_subsample_executable": (
            f"POST-primary: n={N_STATES} dense → RSS/timeout gate (NOT exit 37/-6) → "
            f"n={FALLBACK_N_STATES} dense + classify(feasibility_subsample=True) only when "
            "primary_dense_fork_readable==false"
        ),
        "wrapper_reporting": (
...
def verify_dense_packet_contract(draft: dict[str, Any], replay: dict[str, Any]) -> list[str]:
    ...
    if toggles.get(DENSE_SAMPLED_STATES_ENV) != DENSE_SAMPLED_STATES_VALUE:
        failures.append("dense:env_toggles_missing_sampled_states")
    ...
    if f"export {DENSE_SAMPLED_STATES_ENV}={DENSE_SAMPLED_STATES_VALUE}" not in cmd:
        failures.append("dense:ca_confirmation_command_missing_export")
    if replay.get("primary_command") != cmd:
        failures.append("dense:replay_primary_command_mismatch")
    bce = draft.get("child_emission_contract", {}).get("band_counter_enable", {})
    if bce.get("sampled_states") != DENSE_SAMPLED_STATES_LIST:
        failures.append("dense:band_counter_enable_sampled_states")
    if int(bce.get("expected_mark_events", -1)) != EXPECTED_MARK_COUNT:
        failures.append("dense:expected_mark_events_not_10")
    ...
    if "abort_fail_closed" not in pass_criteria:
        failures.append("dense:pass_criteria_missing_abort_fail_closed")
    outcomes = proof.get("branch_outcomes", {})
    if "PRIMARY_DENSE_FORK_READABLE" not in outcomes:
        failures.append("dense:branch_outcomes_missing_primary_dense_fork_readable")
    obj = draft.get("ca_confirmation_objective", {})
    if obj.get("fork_read_gate_field") != "primary_dense_fork_readable":
        failures.append("dense:objective_missing_fork_read_gate_field")
    if draft.get("task_id") != ACTIVE_TASK_ID:
        failures.append("dense:draft_task_id_not_active_slice")
    if replay.get("task_id") != ACTIVE_TASK_ID:
        failures.append("dense:replay_task_id_not_active_slice")
    return failures
...
def verify_wrapper_receipt_sink_executable() -> list[str]:
    """Assert heredoc writes wrapper receipt and computes primary_dense_fork_readable."""
    failures: list[str] = []
