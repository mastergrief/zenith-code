#!/usr/bin/env python3
"""Thin CLI for the R8 global-cap-relax classifier probe."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Emit the read-only R8 global-cap-relax classifier receipt.",
    )
    ...
    parser.add_argument(
        "--sidecar-relative",
        default="diagnostic/r7_cap_defer_pressure_sidecar.jsonl",
    )
    ...
    receipt = build_classifier_probe_receipt(
        ...
        sidecar_relative=str(args.sidecar_relative),
    )
