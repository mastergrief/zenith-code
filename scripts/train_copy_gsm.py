"""Train copy-augmented HRM on GSM-style word problems.

The acid test: 104+ char inputs, subordinate clauses, 3-4 operands,
multi-step reasoning. The original HRMSeq2Seq hit 28/30 — the copy
mechanism should break through that ceiling.
"""
...
CHECKPOINT_PATH = Path("calm/hrm/checkpoints/copy_gsm_best.pt")


class GSMSeqDataset(Dataset):
    """Pack (GSM problem, expression) into token sequence with loss mask."""
...
def _autoreg_eval(model, problems, device, max_gen=35):
    bos = _CHAR_TO_ID["<bos>"]
    sep = _CHAR_TO_ID["<sep>"]
    eos = _CHAR_TO_ID["<eos>"]
    model.eval()
...
def train(epochs=500, problems=5000, batch_size=64, lr=1e-3,
          ...
          max_len=160, n_copy_heads=4, seed=42, eval_every=10,
    ...
    print(f"[copy-gsm] generated {len(probs)} GSM problems")
    ds = GSMSeqDataset(probs, max_len=max_len)
    ...
    print(f"[copy-gsm] model: {total_params:,} params on {device}")
    if scheduled_sampling:
        print(f"[copy-gsm] scheduled sampling: tf_ratio {tf_ratio_start:.1f}"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            print(f"[copy-gsm] epoch {epoch:4d}/{epochs}: "
    ...
    print(f"[copy-gsm] done: {time.time()-t0:.0f}s, best autoreg={best_autoreg:.1%}")
