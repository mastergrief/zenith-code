"""R22b round 6 — margin-gate the residual write (bug fix from round 5).
...
def main():
    ...
    print(f"[r22b.r6] {len(candidates)} candidates "
    ...
    # --- Install + with-card ---
    print(f"[r22b.r6] installing card (min_margin={MIN_MARGIN})...")
    ...
    slot, state, hook = install(  # type: ignore[name-defined]
        m, card, layer_idx=30, ch_off=2480, write_margin=MIN_MARGIN,
    )
    hook.min_margin = MIN_MARGIN

    CARD_N_RANGE = {5, 10, 15}

    print("[r22b.r6] WITH-CARD pass...")
    t0 = time.time()
    for c in candidates:
