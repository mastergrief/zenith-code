...
from calm.hrm.reasoning_data import ReasoningDataGenerator
from calm.llm_computer.copy_augmented import build_copy_augmented_hrm


CHECKPOINT_PATH = Path("calm/hrm/checkpoints/copy_reasoning_best.pt")
...
def train(epochs=500, problems=5000, batch_size=64, lr=1e-3,
    ...
    print(f"[copy-reason] generated {len(probs)} reasoning problems")
    ...
    print(f"[copy-reason] model: {total_params:,} params on {device}")
    if scheduled_sampling:
        print(f"[copy-reason] scheduled sampling: tf_ratio {tf_ratio_start:.1f}"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            print(f"[copy-reason] epoch {epoch:4d}/{epochs}: "
    ...
    print(f"[copy-reason] done: {time.time()-t0:.0f}s, best autoreg={best_autoreg:.1%}")
