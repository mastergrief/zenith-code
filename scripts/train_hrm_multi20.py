"""Multi-20 HRM training — Experiment 2b (extended distribution probe).

h=32 HRM on 20 input formats. Test whether the scaling curve keeps
climbing past multi10's 50% OOD, or plateaus.
"""
...
from calm.hrm.multi20_data import Multi20Dataset, Multi20Generator


CHECKPOINT_PATH = Path("calm/hrm/checkpoints/multi20_best.pt")


def train(epochs=500, problems=20000, batch_size=128, lr=1e-3, hidden=32,
          num_heads=4, l_layers=1, h_layers=1, dec_layers=1,
          max_enc=128, max_dec=28, seed=42):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[multi20] generating {problems} problems across 20 formats...")
    gen = Multi20Generator(seed=seed)
    probs = gen.generate(problems)
    by_src = {}
    for p in probs:
        by_src.setdefault(p.source, 0)
        by_src[p.source] += 1
    print(f"[multi20] source distribution ({len(by_src)} formats): {dict(sorted(by_src.items()))}")

    ds = Multi20Dataset(probs, max_enc_len=max_enc, max_dec_len=max_dec)
    ...
    print(f"[multi20] model: {model.param_count():,} params on {device}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 50 == 0 or epoch == 1:
            ...
            print(f"[multi20] epoch {epoch:4d}/{epochs}: loss={total/max(nb,1):.4f}, "
            ...
            if va > best:
                ...
                torch.save({
                    ...
                    "epoch": epoch, "val_acc": va, "multi20": True,
                }, CHECKPOINT_PATH)
    print(f"[multi20] done: {time.time()-t0:.0f}s, best val_acc={best:.1%}")
