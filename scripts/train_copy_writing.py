"""Train copy-augmented PT on creative writing function routing.

New output-language family: function routing. The PT maps writing
queries to verification/analysis function names. Mostly generates
function names (not in input) with occasional copy (word arguments).
"""
...
from calm.hrm.writing_data import WritingDataGenerator
from calm.llm_computer.copy_augmented import build_copy_augmented_hrm


CHECKPOINT_PATH = Path("calm/hrm/checkpoints/copy_writing_best.pt")


class WritingSeqDataset(Dataset):
    def __init__(self, problems, max_len=128):
...
def _autoreg_eval(model, problems, device, max_gen=40):
    bos = _CHAR_TO_ID["<bos>"]
    sep = _CHAR_TO_ID["<sep>"]
    eos = _CHAR_TO_ID["<eos>"]
...
def train(epochs=500, problems=5000, batch_size=64, lr=1e-3,
    ...
    gen = WritingDataGenerator(seed=seed)
    probs = gen.generate(problems)
    print(f"[copy-write] generated {len(probs)} writing problems")
    ds = WritingSeqDataset(probs, max_len=max_len)
    ...
    print(f"[copy-write] model: {total_params:,} params on {device}")
    if scheduled_sampling:
        print(f"[copy-write] scheduled sampling: tf_ratio {tf_ratio_start:.1f}"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            print(f"[copy-write] epoch {epoch:4d}/{epochs}: "
    ...
    print(f"[copy-write] done: {time.time()-t0:.0f}s, best autoreg={best_autoreg:.1%}")
