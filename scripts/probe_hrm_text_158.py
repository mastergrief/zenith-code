...
def _build_model_from_ckpt(ckpt: dict, device: str) -> tuple[LMHead, Gsm8kTokenizer]:
...
def _decode_greedy_no_cache(
    ...
    *,
    ...
    Returns (decoded_string, too_long_flag). too_long_flag = True iff
    the prompt prefix `<bos> question <sep>` was longer than max_seq_len,
    so the row could not be evaluated. Caller distinguishes these from
    ordinary wrong decodes.
    """
...
def probe(
    ...
    for i, r in enumerate(val_rows):
        ...
        decoded, too_long = _decode_greedy_no_cache(
    ...
    canonical_decoded, canonical_too_long = _decode_greedy_no_cache(
