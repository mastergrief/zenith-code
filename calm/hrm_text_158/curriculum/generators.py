...
_RUNG_SPEC: dict[str, dict[str, dict]] = {
    ...
    # L0b (codex msg 1779567887201-1cf4f485 +1 Slice D.1 implement second
    # language-axis rung over validated R0..R1b9 math primitives. Single
    # paraphrase template `calculate <math>.`. Identical partition shape
    # to L0a (184 train + 46 held = 230 rows, 13 source-rung buckets;
    # only the template wrapper differs). Audit lives in
    # `language_supports.py` alongside L0a; LANGUAGE_EXPECTED_AGGREGATE
    # extends 230 -> 460. R1b10 excluded from positional priors via
    # DIAGNOSIS_ONLY_RUNGS filter. L0a IS in L0b's positional priors so
    # L0b training preserves L0a paraphrase axis. Spec is a single
    # bounded support, not parametric - partition function
    # `_enumerate_partition_l0b` returns the explicit 230-row list with
    # source_rung metadata for per-bucket reporting parallel to L0a.
    "L0b": {
...
def _enumerate_partition_l0b(seed: int) -> tuple[list[dict], list[dict]]:
    """Bounded stratified L0b support: 184 train + 46 held = 230 total.

    Codex msg 1779567887201-1cf4f485 +1 Slice D.1 implement. L0b mirrors
    L0a's partition shape exactly; only the question-string template
    differs (`calculate <expr>.` instead of `what's <expr>?`). Same
    13-bucket source-rung breakdown, same train/held counts per bucket,
    same deterministic per-bucket seed namespace (`L0b_partition`,
    distinct from L0a's `L0a_partition`).

    Invariants:
      - L0b train ∩ L0b held = ∅ (within-L0b disjoint)
      - L0b expressions drawn only from validated R0..R1b9 SEMANTICS
        (math operation outputs match parent R0..R1b9 expected values)
      - L0b question strings inherently distinct from L0a (`calculate
        ... .` vs `what's ... ?`) AND from any math row (`what is
        ... ?`), so no cross-rung row collisions.
      - All one_digit (R0: A in {0..9}; R1bN: A in {1..9}) exhaustive
        in train
      - two_digit picks per source rung are deterministic via
        `_stable_seed("L0b_partition", seed, "<source_rung>")` —
        DISTINCT from L0a's namespace so the two language rungs pick
        different two_digit slices and the audit gates them
        independently.

    Per-source-rung counts (identical to L0a):
      R0:           20 (10 one_digit + 10 two_digit), 16 train / 4 held
      R1_plus_0:    10 (10 A), 8 train / 2 held
      R1_0_plus_A:  10 (10 A), 8 train / 2 held
      R1_minus_0:   10 (10 A), 8 train / 2 held
    ...
    # one_digit {0..9} exhaustive in train.
    for n in range(0, 10):
        train.append({"question": f"calculate {n}.", "expected": n, "source_rung": "R0"})
    # two_digit: sample 10 stratified from [10, 99], shuffle, split 6 train + 4 held.
    rng_r0 = random.Random(_stable_seed("L0b_partition", seed, "R0_two_digit"))
    r0_two = sorted(rng_r0.sample(range(10, 100), 10))
    rng_r0_split = random.Random(_stable_seed("L0b_partition", seed, "R0_two_split"))
    rng_r0_split.shuffle(r0_two)
    for n in r0_two[:6]:
        train.append({"question": f"calculate {n}.", "expected": n, "source_rung": "R0"})
    for n in r0_two[6:]:
        held.append({"question": f"calculate {n}.", "expected": n, "source_rung": "R0"})

    # R1 identity bridge: 3 sub-templates × 10 A each = 30 rows; 24/6 total
    # (8/2 per sub-template).
    r1_sub_specs = [
        ("R1_plus_0", lambda a: (f"calculate {a} plus 0.", a)),
        ("R1_0_plus_A", lambda a: (f"calculate 0 plus {a}.", a)),
        ("R1_minus_0", lambda a: (f"calculate {a} minus 0.", a)),
    ]
    for sub_name, template_fn in r1_sub_specs:
        rng_sub = random.Random(_stable_seed("L0b_partition", seed, sub_name))
        r1_a = sorted(rng_sub.sample(range(0, 100), 10))
        rng_sub_split = random.Random(_stable_seed("L0b_partition", seed, f"{sub_name}_split"))
        rng_sub_split.shuffle(r1_a)
        for a in r1_a[:8]:
            q, exp = template_fn(a)
            train.append({"question": q, "expected": exp, "source_rung": sub_name})
    ...
    for source_rung, op_str, op_fn, max_a in r1b_specs:
        # one_digit {1..9} exhaustive in train.
        for a in range(1, 10):
            q = f"calculate {a}{op_str}."
            train.append({"question": q, "expected": op_fn(a), "source_rung": source_rung})
        # two_digit pool is [10, max_a].
        rng_two = random.Random(_stable_seed("L0b_partition", seed, f"{source_rung}_two_digit"))
        two_pool = list(range(10, max_a + 1))
        sampled = sorted(rng_two.sample(two_pool, 11))
        rng_split = random.Random(_stable_seed("L0b_partition", seed, f"{source_rung}_split"))
        rng_split.shuffle(sampled)
        for a in sampled[:7]:
            q = f"calculate {a}{op_str}."
            train.append({"question": q, "expected": op_fn(a), "source_rung": source_rung})
    ...
    assert len(train) == 184, f"L0b train size: {len(train)}"
    assert len(held) == 46, f"L0b held size: {len(held)}"
    return train, held


def _gen_l0b(rng: random.Random, spec: dict, n: int, seed: int, split: str) -> list[dict]:
    """L0b paraphrase wrapper training-side sampler (codex msg
    1779567887201-1cf4f485 +1 Slice D.1 implement).

    Samples with replacement from the L0b train/held pool (184/46 rows
    respectively) until n examples are drawn. Strips `source_rung`
    metadata (training side doesn't need it) and adds `rung: "L0b"`.

    Multiplicity at default recipe (n_train=10000, rr=0.65):
      n_new = 3500, unique_train = 184, multiplicity = 19.0x (well
      above 10x floor; matches L0a's recipe under the same flag set).
    """
    train_pool, held_pool = _enumerate_partition_l0b(seed)
    pool = train_pool if split == "train" else held_pool
    out = []
    while len(out) < n:
        row = rng.choice(pool)
        out.append({"question": row["question"], "expected": row["expected"], "rung": "L0b"})
    return out
