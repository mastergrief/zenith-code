"""is_prime(...) via CALM safe_eval oracle.
...
Capability: parse Is_prime queries, evaluate via CALM safe_eval oracle,
...
class Is_primeFacade:
    ...
    OUTPUT_TYPE = 'bool'     # "int" or "bool"
    BOOL_TRUE_STR = 'Yes'
    BOOL_FALSE_STR = 'No'

    _PARSE_RES = [
        re.compile('is_prime\\s*\\(\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
        re.compile('is_prime\\s+of\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('(?:what\\s+is\\s+)?is_prime\\s+(-?\\d+)', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'is_prime({a})'
