"""is_prime(n) — True iff n is prime. Bool output demo (L3).
...
Capability: parse IsPrime queries, evaluate via CALM safe_eval oracle,
...
class IsPrimeFacade:
    ...
    _PARSE_RES = [
        re.compile('is\\s+(-?\\d+)\\s+(?:a\\s+)?prime\\b', re.IGNORECASE),
        re.compile('is_prime\\s*\\(\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
        re.compile('is\\s+(-?\\d+)\\s+prime\\s*\\?', re.IGNORECASE),
    ]
