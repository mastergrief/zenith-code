...
Capability: parse LcmMeta queries, evaluate via CALM safe_eval oracle,
...
class LcmMetaFacade:
    ...
    _PARSE_RES = [
        re.compile('lcm\\s*\\(\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
        re.compile('lcm\\s+of\\s+(-?\\d+)\\s+and\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('(-?\\d+)\\s+lcm\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('lcm\\s+(-?\\d+)\\s+(?:by|with|taken)\\s+(-?\\d+)', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'lcm({a}, {b})'
