...
Capability: parse NextPrime queries, evaluate via CALM safe_eval oracle,
...
class NextPrimeFacade:
    ...
    _PARSE_RES = [
        re.compile('next\\s+prime\\s+(?:after|greater\\s+than|above)\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('smallest\\s+prime\\s+(?:greater|larger|bigger)\\s+than\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('prime\\s+(?:number\\s+)?after\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('next_prime\\s*\\(\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'next_prime({a})'
