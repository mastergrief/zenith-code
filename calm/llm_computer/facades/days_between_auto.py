"""Days between two ISO-formatted dates (YYYY-MM-DD).
...
Capability: parse DaysBetween queries, evaluate via CALM safe_eval oracle,
...
class DaysBetweenFacade:
    ...
    MAX_OPERAND = None
    OPERAND_COUNT = 2
    OPERAND_TYPE = 'str'   # "int" or "str"
    OUTPUT_TYPE = 'int'     # "int" or "bool"
    BOOL_TRUE_STR = 'Yes'
    BOOL_FALSE_STR = 'No'

    _PARSE_RES = [
        re.compile('days\\s+between\\s+(\\d{4}-\\d{2}-\\d{2})\\s+(?:and|to|-)\\s+(\\d{4}-\\d{2}-\\d{2})', re.IGNORECASE),
        re.compile('how\\s+many\\s+days\\s+(?:between|from)\\s+(\\d{4}-\\d{2}-\\d{2})\\s+(?:and|to)\\s+(\\d{4}-\\d{2}-\\d{2})', re.IGNORECASE),
        re.compile('days\\s+from\\s+(\\d{4}-\\d{2}-\\d{2})\\s+to\\s+(\\d{4}-\\d{2}-\\d{2})', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'days_between("{a}", "{b}")'
