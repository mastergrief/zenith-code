"""factorial(...) via CALM safe_eval oracle.
...
Capability: parse FactorialMeta queries, evaluate via CALM safe_eval oracle,
...
class FactorialMetaFacade:
    ...
    _PARSE_RES = [
        re.compile('factorial\\s*\\(\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
        re.compile('factorial\\s+of\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('(?:what\\s+is\\s+)?factorial\\s+(-?\\d+)', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'factorial({a})'
