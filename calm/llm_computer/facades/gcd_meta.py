...
Capability: parse GcdMeta queries, evaluate via CALM safe_eval oracle,
...
class GcdMetaFacade:
    ...
    _PARSE_RES = [
        re.compile('gcd\\s*\\(\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
        re.compile('gcd\\s+of\\s+(-?\\d+)\\s+and\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('(-?\\d+)\\s+gcd\\s+(-?\\d+)', re.IGNORECASE),
        re.compile('gcd\\s+(-?\\d+)\\s+(?:by|with|taken)\\s+(-?\\d+)', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'gcd({a}, {b})'
