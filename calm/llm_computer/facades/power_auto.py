"""pow(a, b) — a raised to the power b, integer exponent.
...
Capability: parse Power queries, evaluate via CALM safe_eval oracle,
...
class PowerFacade:
    ...
    _PARSE_RES = [
        re.compile('(-?\\d+)\\s+to\\s+the\\s+(?:power\\s+(?:of\\s+)?)?(-?\\d+)(?:(?:st|nd|rd|th)\\s+power)?', re.IGNORECASE),
        re.compile('(-?\\d+)\\s*\\^\\s*(-?\\d+)', re.IGNORECASE),
        re.compile('(-?\\d+)\\s*\\*\\*\\s*(-?\\d+)', re.IGNORECASE),
        re.compile('\\bpow\\s*\\(\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\)', re.IGNORECASE),
        re.compile('(-?\\d+)\\s+raised\\s+to\\s+(?:the\\s+)?(-?\\d+)(?:(?:st|nd|rd|th)\\s+power)?', re.IGNORECASE),
    ]
    _EVAL_TEMPLATE = 'pow({a}, {b})'
