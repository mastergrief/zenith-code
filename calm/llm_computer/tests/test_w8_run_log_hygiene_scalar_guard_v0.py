"""Regression: scalar JSON lines in run.log must not crash W8 hygiene counters."""
...
from calm.hrm_text_158.native_full_stack.w8_launch_hygiene import (
...
def test_scale_smoke_hygiene_path_uses_helper_on_fixture(tmp_path: Path) -> None:
    """Scale-smoke assert path: run.log under cheap_complete_w8_on scratch."""
    scratch = tmp_path / "scale_smoke" / "cheap_complete_w8_on"
...
@pytest.mark.parametrize("arm", ["int16_oracle_flag_off", "w8_dense_acc_treatment"])
