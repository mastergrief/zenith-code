...
def test_generator_r1b2_in_rung_names_after_r1b2a() -> None:
    ...
    assert RUNG_NAMES[8] == "R1b5", f"R1b5 must be at index 8 (post-R1b4v2); got {RUNG_NAMES}"
...
#  1779488238721-49f03cc9 R1b4v2 advance + R1b5 direction; 1779523412979-
#  ff88b885 +1 design + +1 implement with keyed per-rung audits)
# ============================================================================ #

def _r1b5_a(ex: dict) -> int:
    """Extract A from an R1b5 example: question is `what is A plus 4?`."""
    q = ex["question"]
    assert q.endswith(" plus 4?"), f"R1b5 question must end ' plus 4?': {q!r}"
    return int(q.split()[2])


def test_generator_r1b5_in_rung_names_index_8() -> None:
    """R1b5 at RUNG_NAMES index 8, immediately after R1b4v2 at 7. Codex
    msg 1779523412979-ff88b885 +1 implement: constant K=4 addition
    extending jigsaw from K=3 (R1b4v2 ADVANCED at b368b81)."""
    from calm.hrm_text_158.curriculum.generators import RUNG_NAMES
    assert RUNG_NAMES[8] == "R1b5", f"R1b5 must be at index 8; got {RUNG_NAMES}"
    assert RUNG_NAMES[7] == "R1b4v2", f"R1b4v2 must be at index 7 (pre-R1b5); got {RUNG_NAMES}"
    assert RUNG_NAMES[9] == "R1b6", f"R1b6 must be at index 9 (post-R1b5); got {RUNG_NAMES}"
    assert RUNG_NAMES[10] == "R1b7", f"R1b7 must be at index 10 (post-R1b6); got {RUNG_NAMES}"
    assert RUNG_NAMES[11] == "R1b8", f"R1b8 must be at index 11 (post-R1b7); got {RUNG_NAMES}"


def test_generator_r1b5_train_holdout_exact_row_disjoint() -> None:
    """R1b5 train + held_out exact-row disjoint at n=2000 sampling."""
    train = make_rung_examples("R1b5", n=2000, seed=42, split="train")
    held = make_rung_examples("R1b5", n=2000, seed=42, split="held_out")
    train_set = {(ex["question"], ex["expected"]) for ex in train}
    held_set = {(ex["question"], ex["expected"]) for ex in held}
    overlap = train_set & held_set
    assert not overlap, f"R1b5 train/held_out share rows: {sorted(overlap)[:5]}"


def test_generator_r1b5_single_template_only() -> None:
    """R1b5 emits ONLY `what is A plus 4?`."""
    rows = make_rung_examples("R1b5", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b5", n=2000, seed=42, split="held_out")
    for ex in rows:
        q = ex["question"]
        assert q.startswith("what is "), f"R1b5 prefix violated: {q!r}"
        assert q.endswith(" plus 4?"), f"R1b5 must end ' plus 4?': {q!r}"
        assert q.count(" plus ") == 1, f"R1b5 must contain ' plus ' exactly once: {q!r}"


def test_generator_r1b5_train_contains_all_one_digit() -> None:
    """R1b5 train MUST contain all 9 one_digit A values [1, 9] (R1b4v2
    lesson baked in: no thin-pool heldout)."""
    train = make_rung_examples("R1b5", n=4000, seed=42, split="train")
    train_as = {_r1b5_a(ex) for ex in train}
    one_digit_as = {a for a in train_as if 1 <= a <= 9}
    assert one_digit_as == set(range(1, 10)), (
        f"R1b5 train missing one_digit values: expected {{1..9}}, "
        f"got {sorted(one_digit_as)}"
    )


def test_generator_r1b5_holdout_has_zero_one_digit() -> None:
    """R1b5 held_out MUST contain ZERO one_digit rows (R1b4v2 lesson:
    heldout is two_digit-only)."""
    held = make_rung_examples("R1b5", n=4000, seed=42, split="held_out")
    held_as = {_r1b5_a(ex) for ex in held}
    one_digit_held = {a for a in held_as if 1 <= a <= 9}
    assert one_digit_held == set(), (
        f"R1b5 held_out leaked one_digit rows: {sorted(one_digit_held)}"
    )


def test_generator_r1b5_holdout_has_18_unique_two_digit() -> None:
    """R1b5 held_out unique support = exactly 18 two_digit A values
    (7 carry + 11 non-carry from carry-stratified 80/20). Codex msg
    1779523412979-ff88b885 spec."""
    held = make_rung_examples("R1b5", n=4000, seed=42, split="held_out")
    held_as = {_r1b5_a(ex) for ex in held}
    assert len(held_as) == 18, (
        f"R1b5 held_out unique A count must be 18; got {len(held_as)}: "
        f"{sorted(held_as)}"
    )
    for A in held_as:
        assert 10 <= A <= 95, f"R1b5 held_out A out of two_digit range: {A}"


def test_generator_r1b5_holdout_carry_stratified_7_plus_11() -> None:
    """R1b5 held_out must contain EXACTLY 7 carry + 11 non-carry
    two_digit values. Carry-stratified 80/20 spec from codex msg
    1779523412979-ff88b885."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b5
    _, held = _enumerate_partition_r1b5(seed=42)
    carry_held = sorted(a for a in held if (a % 10) in (6, 7, 8, 9))
    non_carry_held = sorted(a for a in held if 10 <= a <= 95 and (a % 10) < 6)
    assert len(carry_held) == 7, (
        f"R1b5 carry heldout must be 7; got {len(carry_held)}: {carry_held}"
    )
    assert len(non_carry_held) == 11, (
        f"R1b5 non-carry heldout must be 11; got {len(non_carry_held)}: {non_carry_held}"
    )


def test_generator_r1b5_train_has_77_unique() -> None:
    """R1b5 train unique support = 77 A values (9 one_digit + 25 carry +
    43 non-carry)."""
    train = make_rung_examples("R1b5", n=8000, seed=42, split="train")
    train_as = {_r1b5_a(ex) for ex in train}
    assert len(train_as) == 77, (
        f"R1b5 train unique A count must be 77 (9 one + 25 carry + 43 non-carry); "
        f"got {len(train_as)}"
    )


def test_generator_r1b5_partition_carry_bucket_sizes() -> None:
    """`_enumerate_partition_r1b5` carry bucket: 32 vals -> 25 train + 7 held.
    Non-carry bucket: 54 vals -> 43 train + 11 held."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b5
    train, held = _enumerate_partition_r1b5(seed=42)
    carry_train = {a for a in train if 10 <= a <= 95 and (a % 10) in (6, 7, 8, 9)}
    carry_held = {a for a in held if (a % 10) in (6, 7, 8, 9)}
    non_carry_train = {a for a in train if 10 <= a <= 95 and (a % 10) < 6}
    non_carry_held = {a for a in held if 10 <= a <= 95 and (a % 10) < 6}
    assert len(carry_train) == 25, f"carry train: {len(carry_train)}"
    assert len(carry_held) == 7, f"carry held: {len(carry_held)}"
    assert len(non_carry_train) == 43, f"non-carry train: {len(non_carry_train)}"
    assert len(non_carry_held) == 11, f"non-carry held: {len(non_carry_held)}"
    # Carry + non-carry covers [10, 95] exhaustively
    assert (carry_train | carry_held | non_carry_train | non_carry_held) == set(range(10, 96))


def test_generator_r1b5_expected_matches_arithmetic() -> None:
    """expected == A + 4 for every R1b5 row."""
    rows = make_rung_examples("R1b5", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b5", n=2000, seed=42, split="held_out")
    for ex in rows:
        A = _r1b5_a(ex)
        assert ex["expected"] == A + 4, (
            f"R1b5 arithmetic mismatch: A={A}, expected={ex['expected']}"
        )


def test_generator_r1b5_output_in_5_to_99() -> None:
    """R1b5 outputs in [5, 99] (A in [1, 95] + 4); no 3-digit class."""
    rows = make_rung_examples("R1b5", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b5", n=2000, seed=42, split="held_out")
    for ex in rows:
        assert 5 <= ex["expected"] <= 99, f"R1b5 output OOR: {ex}"


def test_generator_r1b5_rung_field() -> None:
    """Every R1b5 row carries rung='R1b5'."""
    rows = make_rung_examples("R1b5", n=200, seed=42, split="train") + \
           make_rung_examples("R1b5", n=200, seed=42, split="held_out")
    for ex in rows:
        assert ex["rung"] == "R1b5", f"R1b5 rung field violated: {ex!r}"


def test_r1b5_one_digit_audit_returns_9_rows() -> None:
    """The audit accessor returns exactly 9 rows (A in [1, 9] exhaustive)."""
    from calm.hrm_text_158.curriculum import r1b5_one_digit_audit_rows
    rows = r1b5_one_digit_audit_rows(seed=42)
    assert len(rows) == 9, f"audit must return 9 rows; got {len(rows)}"


def test_r1b5_one_digit_audit_seed_invariant() -> None:
    """R1b5 audit rows are seed-invariant (rows ARE the full domain)."""
    from calm.hrm_text_158.curriculum import r1b5_one_digit_audit_rows
    rows_a = r1b5_one_digit_audit_rows(seed=42)
    rows_b = r1b5_one_digit_audit_rows(seed=777)
    rows_c = r1b5_one_digit_audit_rows(seed=0)
    assert rows_a == rows_b == rows_c, (
        "R1b5 audit must be seed-invariant; rows ARE the full domain"
    )


def test_r1b5_one_digit_audit_covers_full_domain() -> None:
    """R1b5 audit covers A in [1, 9] exhaustively; expected = A + 4."""
    from calm.hrm_text_158.curriculum import r1b5_one_digit_audit_rows
    rows = r1b5_one_digit_audit_rows()
    audit_as = sorted(int(r["question"].split()[2]) for r in rows)
    assert audit_as == list(range(1, 10)), f"audit A coverage: {audit_as}"
    for r in rows:
        A = int(r["question"].split()[2])
        assert r["question"] == f"what is {A} plus 4?", f"audit question: {r!r}"
        assert r["expected"] == A + 4, f"audit expected: {r!r}"
        assert r["rung"] == "R1b5", f"audit rung field: {r!r}"


def test_r1b5_audit_disjoint_from_holdout() -> None:
    """R1b5 9-row one_digit audit must be disjoint from R1b5 heldout
    (which is two_digit-only). Sanity check on partition design."""
    from calm.hrm_text_158.curriculum import r1b5_one_digit_audit_rows
    audit_rows = r1b5_one_digit_audit_rows()
    held = make_rung_examples("R1b5", n=4000, seed=42, split="held_out")
    audit_qs = {r["question"] for r in audit_rows}
    held_qs = {ex["question"] for ex in held}
    overlap = audit_qs & held_qs
    assert not overlap, f"R1b5 audit ∩ heldout: {overlap}"


def test_r1b5_default_chain_no_overlap() -> None:
    """Default active chain (with R1b5) passes cross-rung train/held_out
    invariant. B=4 disjoint from priors R0/R1/R1b1/R1b2/R1b3/R1b4v2."""
    splits = build_rung_splits(
        n_train=2000,
        n_held_out=400,
        seed=42,
        rungs=("R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5"),
    )
    assert_no_train_holdout_overlap(splits)


@pytest.mark.parametrize("diagnosis_rung", ["R2a", "R2"])
def test_cross_rung_r1b5_and_r2_together_collide(diagnosis_rung) -> None:
    """Negative test: R1b5 (B=4) overlaps diagnosis-only R2/R2a (both
    use B in [2,9] templates and A in [10,19]) for B=4 subset.
    Parametrized over both R2a and R2 per codex msg 1779524217097-3611598a
    cleanup. R2/R2a stay diagnosis-only; cross-rung invariant catches
    the collision only when both opted in."""
    splits = build_rung_splits(
        n_train=2000,
        n_held_out=400,
        seed=42,
        rungs=("R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", diagnosis_rung),
    )
    with pytest.raises(AssertionError, match="overlap detected"):
        assert_no_train_holdout_overlap(splits)


def test_r1b5_partition_one_digit_exhaustive() -> None:
    """`_enumerate_partition_r1b5` puts ALL 9 one_digit values in train
    and ZERO in heldout (R1b4v2 lesson)."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b5
    train, held = _enumerate_partition_r1b5(seed=42)
    one_digit_train = {a for a in train if 1 <= a <= 9}
    one_digit_held = {a for a in held if 1 <= a <= 9}
...
def test_r1b5_partition_stable_across_pythonhashseed() -> None:
    """`_enumerate_partition_r1b5` is stable across PYTHONHASHSEED (uses
    `_stable_seed` for carry + non_carry shuffles; one_digit exhaustive)."""
    import os
    import subprocess
    import sys
    snippet = (
    ...
    out2 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "777"}).decode().strip()
    out3 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "random"}).decode().strip()
    assert out1 == out2 == out3, "R1b5 partition diverged across PYTHONHASHSEED"


# ============================================================================ #
# R1b6 constant K=5 addition (codex msg 1779545956176-4a8cfc3e +1 K=5 naming
#  + 1779545998312-88256068 prior_rungs receipt addendum after gabe greenlight
#  relay 1779545575582-7c52a912 of verbatim "ok implement, full prov").
#  Continues constant-K jigsaw (K=1, K=-1, K=2, K=3, K=4 all PASSED).
#  Mirrors R1b5 carry-stratified design at K=5.
# ============================================================================ #

def _r1b6_a(ex: dict) -> int:
    """Extract A from an R1b6 example: question is `what is A plus 5?`."""
    q = ex["question"]
    assert q.startswith("what is "), f"R1b6 prefix violated: {q!r}"
    assert q.endswith(" plus 5?"), f"R1b6 question must end ' plus 5?': {q!r}"
    return int(q[len("what is "):-len(" plus 5?")])


def test_generator_r1b6_in_rung_names_index_9() -> None:
    """R1b6 at RUNG_NAMES index 9, immediately after R1b5 at 8, before R1b7
    at 10. Codex msg 1779545956176-4a8cfc3e: constant K=5 addition extending
    jigsaw from K=4 (R1b5 ADVANCED via seed=17); R1b7 inserted at 10 per
    codex msg 1779547753761-5711d790."""
    from calm.hrm_text_158.curriculum.generators import RUNG_NAMES
    assert RUNG_NAMES[9] == "R1b6", f"R1b6 must be at index 9; got {RUNG_NAMES}"
    assert RUNG_NAMES[8] == "R1b5", f"R1b5 must be at index 8 (pre-R1b6); got {RUNG_NAMES}"
    assert RUNG_NAMES[10] == "R1b7", f"R1b7 must be at index 10 (post-R1b6); got {RUNG_NAMES}"


def test_generator_r1b6_train_holdout_exact_row_disjoint() -> None:
    """R1b6 train + held_out exact-row disjoint at n=2000 sampling."""
    train = make_rung_examples("R1b6", n=2000, seed=42, split="train")
    held = make_rung_examples("R1b6", n=2000, seed=42, split="held_out")
    train_set = {(ex["question"], ex["expected"]) for ex in train}
    held_set = {(ex["question"], ex["expected"]) for ex in held}
    overlap = train_set & held_set
    assert not overlap, f"R1b6 train/held_out share rows: {sorted(overlap)[:5]}"


def test_generator_r1b6_single_template_only() -> None:
    """R1b6 emits ONLY `what is A plus 5?`."""
    rows = make_rung_examples("R1b6", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b6", n=2000, seed=42, split="held_out")
    for ex in rows:
        q = ex["question"]
        assert q.startswith("what is "), f"R1b6 prefix violated: {q!r}"
        assert q.endswith(" plus 5?"), f"R1b6 must end ' plus 5?': {q!r}"
        assert q.count(" plus ") == 1, f"R1b6 must contain ' plus ' exactly once: {q!r}"


def test_generator_r1b6_train_contains_all_one_digit() -> None:
    """R1b6 train MUST contain all 9 one_digit A values [1, 9] (R1b4v2/R1b5
    lesson baked in: no thin-pool heldout)."""
    train = make_rung_examples("R1b6", n=4000, seed=42, split="train")
    train_as = {_r1b6_a(ex) for ex in train}
    one_digit_as = {a for a in train_as if 1 <= a <= 9}
    assert one_digit_as == set(range(1, 10)), (
        f"R1b6 train missing one_digit values: expected {{1..9}}, "
        f"got {sorted(one_digit_as)}"
    )


def test_generator_r1b6_holdout_has_zero_one_digit() -> None:
    """R1b6 held_out MUST contain ZERO one_digit rows (R1b4v2/R1b5 lesson:
    heldout is two_digit-only)."""
    held = make_rung_examples("R1b6", n=4000, seed=42, split="held_out")
    held_as = {_r1b6_a(ex) for ex in held}
    one_digit_held = {a for a in held_as if 1 <= a <= 9}
    assert one_digit_held == set(), (
        f"R1b6 held_out leaked one_digit rows: {sorted(one_digit_held)}"
    )


def test_generator_r1b6_holdout_has_17_unique_two_digit() -> None:
    """R1b6 held_out unique support = exactly 17 two_digit A values
    (8 carry + 9 non-carry from carry-stratified 80/20 at K=5). Codex msg
    1779545956176-4a8cfc3e spec."""
    held = make_rung_examples("R1b6", n=4000, seed=42, split="held_out")
    held_as = {_r1b6_a(ex) for ex in held}
    assert len(held_as) == 17, (
        f"R1b6 held_out unique A count must be 17; got {len(held_as)}: "
        f"{sorted(held_as)}"
    )
    for A in held_as:
        assert 10 <= A <= 94, f"R1b6 held_out A out of two_digit range: {A}"


def test_generator_r1b6_holdout_carry_stratified_8_plus_9() -> None:
    """R1b6 held_out must contain EXACTLY 8 carry + 9 non-carry two_digit
    values. Carry-stratified 80/20 at K=5 (carry-units {5..9} since K=5),
    spec from codex msg 1779545956176-4a8cfc3e."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b6
    _, held = _enumerate_partition_r1b6(seed=42)
    carry_held = sorted(a for a in held if (a % 10) in (5, 6, 7, 8, 9))
    non_carry_held = sorted(a for a in held if 10 <= a <= 94 and (a % 10) < 5)
    assert len(carry_held) == 8, (
        f"R1b6 carry heldout must be 8; got {len(carry_held)}: {carry_held}"
    )
    assert len(non_carry_held) == 9, (
        f"R1b6 non-carry heldout must be 9; got {len(non_carry_held)}: {non_carry_held}"
    )


def test_generator_r1b6_train_has_77_unique() -> None:
    """R1b6 train unique support = 77 A values (9 one_digit + 32 carry +
    36 non-carry)."""
    train = make_rung_examples("R1b6", n=8000, seed=42, split="train")
    train_as = {_r1b6_a(ex) for ex in train}
    assert len(train_as) == 77, (
        f"R1b6 train unique A count must be 77 (9 one + 32 carry + 36 non-carry); "
        f"got {len(train_as)}"
    )


def test_generator_r1b6_partition_carry_bucket_sizes() -> None:
    """`_enumerate_partition_r1b6` carry bucket: 40 vals -> 32 train + 8 held.
    Non-carry bucket: 45 vals -> 36 train + 9 held."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b6
    train, held = _enumerate_partition_r1b6(seed=42)
    carry_train = {a for a in train if 10 <= a <= 94 and (a % 10) in (5, 6, 7, 8, 9)}
    carry_held = {a for a in held if (a % 10) in (5, 6, 7, 8, 9)}
    non_carry_train = {a for a in train if 10 <= a <= 94 and (a % 10) < 5}
    non_carry_held = {a for a in held if 10 <= a <= 94 and (a % 10) < 5}
    assert len(carry_train) == 32, f"carry train: {len(carry_train)}"
    assert len(carry_held) == 8, f"carry held: {len(carry_held)}"
    assert len(non_carry_train) == 36, f"non-carry train: {len(non_carry_train)}"
    assert len(non_carry_held) == 9, f"non-carry held: {len(non_carry_held)}"
    # Carry + non-carry covers [10, 94] exhaustively (85 vals)
    assert (carry_train | carry_held | non_carry_train | non_carry_held) == set(range(10, 95))


def test_generator_r1b6_expected_matches_arithmetic() -> None:
    """expected == A + 5 for every R1b6 row."""
    rows = make_rung_examples("R1b6", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b6", n=2000, seed=42, split="held_out")
    for ex in rows:
        A = _r1b6_a(ex)
        assert ex["expected"] == A + 5, (
            f"R1b6 arithmetic mismatch: A={A}, expected={ex['expected']}"
        )


def test_generator_r1b6_output_in_6_to_99() -> None:
    """R1b6 outputs in [6, 99] (A in [1, 94] + 5); no 3-digit class."""
    rows = make_rung_examples("R1b6", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b6", n=2000, seed=42, split="held_out")
    for ex in rows:
        assert 6 <= ex["expected"] <= 99, f"R1b6 output OOR: {ex}"


def test_generator_r1b6_rung_field() -> None:
    """Every R1b6 row carries rung='R1b6'."""
    rows = make_rung_examples("R1b6", n=200, seed=42, split="train") + \
           make_rung_examples("R1b6", n=200, seed=42, split="held_out")
    for ex in rows:
        assert ex["rung"] == "R1b6", f"R1b6 rung field violated: {ex!r}"


def test_r1b6_one_digit_audit_returns_9_rows() -> None:
    """The audit accessor returns exactly 9 rows (A in [1, 9] exhaustive)."""
    from calm.hrm_text_158.curriculum import r1b6_one_digit_audit_rows
    rows = r1b6_one_digit_audit_rows(seed=42)
    assert len(rows) == 9, f"audit must return 9 rows; got {len(rows)}"


def test_r1b6_one_digit_audit_seed_invariant() -> None:
    """R1b6 audit rows are seed-invariant (rows ARE the full domain)."""
    from calm.hrm_text_158.curriculum import r1b6_one_digit_audit_rows
    rows_a = r1b6_one_digit_audit_rows(seed=42)
    rows_b = r1b6_one_digit_audit_rows(seed=777)
    rows_c = r1b6_one_digit_audit_rows(seed=0)
    assert rows_a == rows_b == rows_c, (
        "R1b6 audit must be seed-invariant; rows ARE the full domain"
    )


def test_r1b6_one_digit_audit_covers_full_domain() -> None:
    """R1b6 audit covers A in [1, 9] exhaustively; expected = A + 5."""
    from calm.hrm_text_158.curriculum import r1b6_one_digit_audit_rows
    rows = r1b6_one_digit_audit_rows()
    audit_as = sorted(int(r["question"].split()[2]) for r in rows)
    assert audit_as == list(range(1, 10)), f"audit A coverage: {audit_as}"
    for r in rows:
        A = int(r["question"].split()[2])
        assert r["question"] == f"what is {A} plus 5?", f"audit question: {r!r}"
        assert r["expected"] == A + 5, f"audit expected: {r!r}"
        assert r["rung"] == "R1b6", f"audit rung field: {r!r}"


def test_r1b6_one_digit_audit_importable_from_package() -> None:
    """`r1b6_one_digit_audit_rows` is exported from the curriculum package
    (mirrors r1b5 export so probe registry import path works)."""
    from calm.hrm_text_158.curriculum import r1b6_one_digit_audit_rows
    assert callable(r1b6_one_digit_audit_rows)


def test_r1b6_audit_disjoint_from_holdout() -> None:
    """R1b6 9-row one_digit audit must be disjoint from R1b6 heldout
    (which is two_digit-only). Sanity check on partition design."""
    from calm.hrm_text_158.curriculum import r1b6_one_digit_audit_rows
    audit_rows = r1b6_one_digit_audit_rows()
    held = make_rung_examples("R1b6", n=4000, seed=42, split="held_out")
    audit_qs = {r["question"] for r in audit_rows}
    held_qs = {ex["question"] for ex in held}
    overlap = audit_qs & held_qs
    assert not overlap, f"R1b6 audit ∩ heldout: {overlap}"


def test_r1b6_default_chain_contains_r1b6_no_overlap() -> None:
    """Default active chain (with R1b6) passes cross-rung train/held_out
    invariant. B=5 disjoint from priors R0/R1/R1b1/R1b2/R1b3/R1b4v2/R1b5."""
    splits = build_rung_splits(
        n_train=2000,
        n_held_out=400,
        seed=42,
        rungs=("R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", "R1b6"),
    )
    assert "R1b6" in splits, f"R1b6 missing from default chain: {list(splits)}"
    assert_no_train_holdout_overlap(splits)


def test_r1b6_build_rung_splits_default_includes_r1b6() -> None:
    """`build_rung_splits()` with no rungs arg returns the active chain
    that NOW INCLUDES R1b6 (post codex msg 1779545956176-4a8cfc3e)."""
    splits = build_rung_splits(n_train=200, n_held_out=80, seed=42)
    assert "R1b6" in splits, (
        f"R1b6 must be in default active chain; got rungs={list(splits)}"
    )


def test_r1b6_partition_one_digit_exhaustive() -> None:
    """`_enumerate_partition_r1b6` puts ALL 9 one_digit values in train
    and ZERO in heldout (R1b4v2/R1b5 lesson)."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b6
    train, held = _enumerate_partition_r1b6(seed=42)
    one_digit_train = {a for a in train if 1 <= a <= 9}
    one_digit_held = {a for a in held if 1 <= a <= 9}
...
def test_r1b6_partition_train_held_disjoint() -> None:
    """`_enumerate_partition_r1b6` produces strictly disjoint train + held
    sets (sanity vs cross-rung invariant; codex spec
    1779545956176-4a8cfc3e)."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b6
    train, held = _enumerate_partition_r1b6(seed=17)
    assert train.isdisjoint(held), (
        f"R1b6 partition train ∩ held: {sorted(train & held)}"
    )


def test_r1b6_partition_stable_across_pythonhashseed() -> None:
    """`_enumerate_partition_r1b6` is stable across PYTHONHASHSEED (uses
    `_stable_seed` for carry + non_carry shuffles; one_digit exhaustive)."""
    import os
    import subprocess
    import sys
    ...
    out2 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "777"}).decode().strip()
    out3 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "random"}).decode().strip()
    assert out1 == out2 == out3, "R1b6 partition diverged across PYTHONHASHSEED"


def test_r1b6_in_one_digit_audit_registry() -> None:
    """Probe script registers R1b6 in ONE_DIGIT_AUDIT_REGISTRY (codex msg
    1779545956176-4a8cfc3e: keyed per-rung audits required for retention
    visibility through replay-curriculum probes)."""
    from scripts.probe_hrm_text_158 import ONE_DIGIT_AUDIT_REGISTRY
    assert "R1b6" in ONE_DIGIT_AUDIT_REGISTRY, (
        f"R1b6 missing from ONE_DIGIT_AUDIT_REGISTRY: {list(ONE_DIGIT_AUDIT_REGISTRY)}"
    )
    # Registry callable returns the 9 audit rows
    rows = ONE_DIGIT_AUDIT_REGISTRY["R1b6"](seed=42)
    assert len(rows) == 9
    for r in rows:
        assert r["rung"] == "R1b6"


def test_r1b6_positional_priors_match_active_chain() -> None:
    """Trainer's `_resolve_prior_rungs("R1b6")` returns the active chain
    {R0, R1, R1b1, R1b2, R1b3, R1b4v2, R1b5} (diagnosis-only auto-excluded
    per codex msg 1779545998312-88256068 prior_rungs receipt addendum)."""
    from calm.hrm_text_158.curriculum.replay import _resolve_prior_rungs
    priors = _resolve_prior_rungs("R1b6", replay_rungs_arg=None)
    assert priors == ["R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5"], (
        f"R1b6 priors: {priors}"
    )
...
# ============================================================================ #
# R1b7 constant K=6 addition (codex msg 1779547753761-5711d790 +1 K=6 after
#  R1b6 commit 128b097 PASS-via-replay50; durable gabe provenance relay
#  1779547541812-46e20177 of verbatim "you have full provenance until im back").
#  Continues constant-K jigsaw (K=1, K=-1, K=2, K=3, K=4, K=5 all PASSED).
#  Mirrors R1b5/R1b6 carry-stratified design at K=6.
# ============================================================================ #

def _r1b7_a(ex: dict) -> int:
    """Extract A from an R1b7 example: question is `what is A plus 6?`."""
    q = ex["question"]
    assert q.startswith("what is "), f"R1b7 prefix violated: {q!r}"
    assert q.endswith(" plus 6?"), f"R1b7 question must end ' plus 6?': {q!r}"
    return int(q[len("what is "):-len(" plus 6?")])


def test_generator_r1b7_in_rung_names_index_10() -> None:
    """R1b7 at RUNG_NAMES index 10, immediately after R1b6 at 9, before R1b8
    at 11. Codex msg 1779547753761-5711d790: constant K=6 addition extending
    jigsaw from K=5 (R1b6 ADVANCED at 128b097 via replay50_lr5e4); R1b8
    inserted at 11 per codex msg 1779550489408-f40f66ab."""
    from calm.hrm_text_158.curriculum.generators import RUNG_NAMES
    assert RUNG_NAMES[10] == "R1b7", f"R1b7 must be at index 10; got {RUNG_NAMES}"
    assert RUNG_NAMES[9] == "R1b6", f"R1b6 must be at index 9 (pre-R1b7); got {RUNG_NAMES}"
    assert RUNG_NAMES[11] == "R1b8", f"R1b8 must be at index 11 (post-R1b7); got {RUNG_NAMES}"


def test_generator_r1b7_train_holdout_exact_row_disjoint() -> None:
    """R1b7 train + held_out exact-row disjoint at n=2000 sampling."""
    train = make_rung_examples("R1b7", n=2000, seed=42, split="train")
    held = make_rung_examples("R1b7", n=2000, seed=42, split="held_out")
    train_set = {(ex["question"], ex["expected"]) for ex in train}
    held_set = {(ex["question"], ex["expected"]) for ex in held}
    overlap = train_set & held_set
    assert not overlap, f"R1b7 train/held_out share rows: {sorted(overlap)[:5]}"


def test_generator_r1b7_single_template_only() -> None:
    """R1b7 emits ONLY `what is A plus 6?`."""
    rows = make_rung_examples("R1b7", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b7", n=2000, seed=42, split="held_out")
    for ex in rows:
        q = ex["question"]
        assert q.startswith("what is "), f"R1b7 prefix violated: {q!r}"
        assert q.endswith(" plus 6?"), f"R1b7 must end ' plus 6?': {q!r}"
        assert q.count(" plus ") == 1, f"R1b7 must contain ' plus ' exactly once: {q!r}"


def test_generator_r1b7_train_contains_all_one_digit() -> None:
    """R1b7 train MUST contain all 9 one_digit A values [1, 9] (R1b4v2/R1b5/R1b6
    lesson baked in: no thin-pool heldout)."""
    train = make_rung_examples("R1b7", n=4000, seed=42, split="train")
    train_as = {_r1b7_a(ex) for ex in train}
    one_digit_as = {a for a in train_as if 1 <= a <= 9}
    assert one_digit_as == set(range(1, 10)), (
        f"R1b7 train missing one_digit values: expected {{1..9}}, "
        f"got {sorted(one_digit_as)}"
    )


def test_generator_r1b7_holdout_has_zero_one_digit() -> None:
    """R1b7 held_out MUST contain ZERO one_digit rows (R1b4v2/R1b5/R1b6 lesson:
    heldout is two_digit-only)."""
    held = make_rung_examples("R1b7", n=4000, seed=42, split="held_out")
    held_as = {_r1b7_a(ex) for ex in held}
    one_digit_held = {a for a in held_as if 1 <= a <= 9}
    assert one_digit_held == set(), (
        f"R1b7 held_out leaked one_digit rows: {sorted(one_digit_held)}"
    )


def test_generator_r1b7_holdout_has_18_unique_two_digit() -> None:
    """R1b7 held_out unique support = exactly 18 two_digit A values
    (10 carry + 8 non-carry from carry-stratified 80/20 at K=6). Codex msg
    1779547753761-5711d790 spec."""
    held = make_rung_examples("R1b7", n=4000, seed=42, split="held_out")
    held_as = {_r1b7_a(ex) for ex in held}
    assert len(held_as) == 18, (
        f"R1b7 held_out unique A count must be 18; got {len(held_as)}: "
        f"{sorted(held_as)}"
    )
    for A in held_as:
        assert 10 <= A <= 93, f"R1b7 held_out A out of two_digit range: {A}"


def test_generator_r1b7_holdout_carry_stratified_10_plus_8() -> None:
    """R1b7 held_out must contain EXACTLY 10 carry + 8 non-carry two_digit
    values. Carry-stratified 80/20 at K=6 (carry-units {4..9} since K=6),
    spec from codex msg 1779547753761-5711d790."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b7
    _, held = _enumerate_partition_r1b7(seed=42)
    carry_held = sorted(a for a in held if (a % 10) in (4, 5, 6, 7, 8, 9))
    non_carry_held = sorted(a for a in held if 10 <= a <= 93 and (a % 10) < 4)
    assert len(carry_held) == 10, (
        f"R1b7 carry heldout must be 10; got {len(carry_held)}: {carry_held}"
    )
    assert len(non_carry_held) == 8, (
        f"R1b7 non-carry heldout must be 8; got {len(non_carry_held)}: {non_carry_held}"
    )


def test_generator_r1b7_train_has_75_unique() -> None:
    """R1b7 train unique support = 75 A values (9 one_digit + 38 carry +
    28 non-carry)."""
    train = make_rung_examples("R1b7", n=8000, seed=42, split="train")
    train_as = {_r1b7_a(ex) for ex in train}
    assert len(train_as) == 75, (
        f"R1b7 train unique A count must be 75 (9 one + 38 carry + 28 non-carry); "
        f"got {len(train_as)}"
    )


def test_generator_r1b7_partition_carry_bucket_sizes() -> None:
    """`_enumerate_partition_r1b7` carry bucket: 48 vals -> 38 train + 10 held.
    Non-carry bucket: 36 vals -> 28 train + 8 held."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b7
    train, held = _enumerate_partition_r1b7(seed=42)
    carry_train = {a for a in train if 10 <= a <= 93 and (a % 10) in (4, 5, 6, 7, 8, 9)}
    carry_held = {a for a in held if (a % 10) in (4, 5, 6, 7, 8, 9)}
    non_carry_train = {a for a in train if 10 <= a <= 93 and (a % 10) < 4}
    non_carry_held = {a for a in held if 10 <= a <= 93 and (a % 10) < 4}
    assert len(carry_train) == 38, f"carry train: {len(carry_train)}"
    assert len(carry_held) == 10, f"carry held: {len(carry_held)}"
    assert len(non_carry_train) == 28, f"non-carry train: {len(non_carry_train)}"
    assert len(non_carry_held) == 8, f"non-carry held: {len(non_carry_held)}"
    # Carry + non-carry covers [10, 93] exhaustively (84 vals)
    assert (carry_train | carry_held | non_carry_train | non_carry_held) == set(range(10, 94))


def test_generator_r1b7_expected_matches_arithmetic() -> None:
    """expected == A + 6 for every R1b7 row."""
    rows = make_rung_examples("R1b7", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b7", n=2000, seed=42, split="held_out")
    for ex in rows:
        A = _r1b7_a(ex)
        assert ex["expected"] == A + 6, (
            f"R1b7 arithmetic mismatch: A={A}, expected={ex['expected']}"
        )


def test_generator_r1b7_output_in_7_to_99() -> None:
    """R1b7 outputs in [7, 99] (A in [1, 93] + 6); no 3-digit class."""
    rows = make_rung_examples("R1b7", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b7", n=2000, seed=42, split="held_out")
    for ex in rows:
        assert 7 <= ex["expected"] <= 99, f"R1b7 output OOR: {ex}"


def test_generator_r1b7_rung_field() -> None:
    """Every R1b7 row carries rung='R1b7'."""
    rows = make_rung_examples("R1b7", n=200, seed=42, split="train") + \
           make_rung_examples("R1b7", n=200, seed=42, split="held_out")
    for ex in rows:
        assert ex["rung"] == "R1b7", f"R1b7 rung field violated: {ex!r}"


def test_r1b7_one_digit_audit_returns_9_rows() -> None:
    """The audit accessor returns exactly 9 rows (A in [1, 9] exhaustive)."""
    from calm.hrm_text_158.curriculum import r1b7_one_digit_audit_rows
    rows = r1b7_one_digit_audit_rows(seed=42)
    assert len(rows) == 9, f"audit must return 9 rows; got {len(rows)}"


def test_r1b7_one_digit_audit_seed_invariant() -> None:
    """R1b7 audit rows are seed-invariant (rows ARE the full domain)."""
    from calm.hrm_text_158.curriculum import r1b7_one_digit_audit_rows
    rows_a = r1b7_one_digit_audit_rows(seed=42)
    rows_b = r1b7_one_digit_audit_rows(seed=777)
    rows_c = r1b7_one_digit_audit_rows(seed=0)
    assert rows_a == rows_b == rows_c, (
        "R1b7 audit must be seed-invariant; rows ARE the full domain"
    )


def test_r1b7_one_digit_audit_covers_full_domain() -> None:
    """R1b7 audit covers A in [1, 9] exhaustively; expected = A + 6."""
    from calm.hrm_text_158.curriculum import r1b7_one_digit_audit_rows
    rows = r1b7_one_digit_audit_rows()
    audit_as = sorted(int(r["question"].split()[2]) for r in rows)
    assert audit_as == list(range(1, 10)), f"audit A coverage: {audit_as}"
    for r in rows:
        A = int(r["question"].split()[2])
        assert r["question"] == f"what is {A} plus 6?", f"audit question: {r!r}"
        assert r["expected"] == A + 6, f"audit expected: {r!r}"
        assert r["rung"] == "R1b7", f"audit rung field: {r!r}"


def test_r1b7_one_digit_audit_importable_from_package() -> None:
    """`r1b7_one_digit_audit_rows` is exported from the curriculum package
    (mirrors r1b5/r1b6 export so probe registry import path works)."""
    from calm.hrm_text_158.curriculum import r1b7_one_digit_audit_rows
    assert callable(r1b7_one_digit_audit_rows)


def test_r1b7_audit_disjoint_from_holdout() -> None:
    """R1b7 9-row one_digit audit must be disjoint from R1b7 heldout
    (which is two_digit-only). Sanity check on partition design."""
    from calm.hrm_text_158.curriculum import r1b7_one_digit_audit_rows
    audit_rows = r1b7_one_digit_audit_rows()
    held = make_rung_examples("R1b7", n=4000, seed=42, split="held_out")
    audit_qs = {r["question"] for r in audit_rows}
    held_qs = {ex["question"] for ex in held}
    overlap = audit_qs & held_qs
    assert not overlap, f"R1b7 audit ∩ heldout: {overlap}"


def test_r1b7_default_chain_contains_r1b7_no_overlap() -> None:
    """Default active chain (with R1b7) passes cross-rung train/held_out
    invariant. B=6 disjoint from priors R0/R1/R1b1/R1b2/R1b3/R1b4v2/R1b5/R1b6."""
    splits = build_rung_splits(
        n_train=2000,
        n_held_out=400,
        seed=42,
        rungs=("R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", "R1b6", "R1b7"),
    )
    assert "R1b7" in splits, f"R1b7 missing from default chain: {list(splits)}"
    assert_no_train_holdout_overlap(splits)


def test_r1b7_build_rung_splits_default_includes_r1b7() -> None:
    """`build_rung_splits()` with no rungs arg returns the active chain
    that NOW INCLUDES R1b7 (post codex msg 1779547753761-5711d790)."""
    splits = build_rung_splits(n_train=200, n_held_out=80, seed=42)
    assert "R1b7" in splits, (
        f"R1b7 must be in default active chain; got rungs={list(splits)}"
    )


def test_r1b7_partition_one_digit_exhaustive() -> None:
    """`_enumerate_partition_r1b7` puts ALL 9 one_digit values in train
    and ZERO in heldout (R1b4v2/R1b5/R1b6 lesson)."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b7
    train, held = _enumerate_partition_r1b7(seed=42)
    one_digit_train = {a for a in train if 1 <= a <= 9}
    one_digit_held = {a for a in held if 1 <= a <= 9}
...
def test_r1b7_partition_train_held_disjoint() -> None:
    """`_enumerate_partition_r1b7` produces strictly disjoint train + held
    sets (sanity vs cross-rung invariant; codex spec 1779547753761-5711d790)."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b7
    train, held = _enumerate_partition_r1b7(seed=17)
    assert train.isdisjoint(held), (
        f"R1b7 partition train ∩ held: {sorted(train & held)}"
    )


def test_r1b7_partition_stable_across_pythonhashseed() -> None:
    """`_enumerate_partition_r1b7` is stable across PYTHONHASHSEED (uses
    `_stable_seed` for carry + non_carry shuffles; one_digit exhaustive)."""
    import os
    import subprocess
    import sys
    ...
    out2 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "777"}).decode().strip()
    out3 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "random"}).decode().strip()
    assert out1 == out2 == out3, "R1b7 partition diverged across PYTHONHASHSEED"


def test_r1b7_in_one_digit_audit_registry() -> None:
    """Probe script registers R1b7 in ONE_DIGIT_AUDIT_REGISTRY (codex msg
    1779547753761-5711d790: keyed per-rung audits required for retention
    visibility through replay-curriculum probes)."""
    from scripts.probe_hrm_text_158 import ONE_DIGIT_AUDIT_REGISTRY
    assert "R1b7" in ONE_DIGIT_AUDIT_REGISTRY, (
        f"R1b7 missing from ONE_DIGIT_AUDIT_REGISTRY: {list(ONE_DIGIT_AUDIT_REGISTRY)}"
    )
    # Registry callable returns the 9 audit rows
    rows = ONE_DIGIT_AUDIT_REGISTRY["R1b7"](seed=42)
    assert len(rows) == 9
    for r in rows:
        assert r["rung"] == "R1b7"


def test_r1b7_positional_priors_match_active_chain() -> None:
    """Trainer's `_resolve_prior_rungs("R1b7")` returns the active chain
    {R0, R1, R1b1, R1b2, R1b3, R1b4v2, R1b5, R1b6} (diagnosis-only auto-
    excluded per codex msg 1779547753761-5711d790 full-active-chain probe
    spec)."""
    from calm.hrm_text_158.curriculum.replay import _resolve_prior_rungs
    priors = _resolve_prior_rungs("R1b7", replay_rungs_arg=None)
    assert priors == ["R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", "R1b6"], (
        f"R1b7 priors: {priors}"
    )


# ============================================================================ #
# R1b8 constant K=7 addition (codex msg 1779550489408-f40f66ab +1 K=7 after
#  R1b7 commit 682659b ADVANCED via R1b2-retained chain + A0 exhaustive
#  audit 1071/1072 = 99.91% PASS). Continues constant-K jigsaw
#  (K=1, K=-1, K=2, K=3, K=4, K=5, K=6 all PASSED). Mirrors R1b5/R1b6/R1b7
#  carry-stratified design at K=7. R1b2 boundary watch explicit per codex
#  R1b8 launch +1.
# ============================================================================ #

def _r1b8_a(ex: dict) -> int:
    """Extract A from an R1b8 example: question is `what is A plus 7?`."""
    q = ex["question"]
    assert q.startswith("what is "), f"R1b8 prefix violated: {q!r}"
    assert q.endswith(" plus 7?"), f"R1b8 question must end ' plus 7?': {q!r}"
    return int(q[len("what is "):-len(" plus 7?")])


def test_generator_r1b8_in_rung_names_index_11() -> None:
    """R1b8 at RUNG_NAMES index 11, immediately after R1b7 at 10, before R1b
    at 12. Codex msg 1779550489408-f40f66ab."""
    from calm.hrm_text_158.curriculum.generators import RUNG_NAMES
...
def test_generator_r1b8_train_holdout_exact_row_disjoint() -> None:
    """R1b8 train + held_out exact-row disjoint at n=2000 sampling."""
    train = make_rung_examples("R1b8", n=2000, seed=42, split="train")
    held = make_rung_examples("R1b8", n=2000, seed=42, split="held_out")
    train_set = {(ex["question"], ex["expected"]) for ex in train}
    held_set = {(ex["question"], ex["expected"]) for ex in held}
    overlap = train_set & held_set
    assert not overlap, f"R1b8 train/held_out share rows: {sorted(overlap)[:5]}"


def test_generator_r1b8_single_template_only() -> None:
    """R1b8 emits ONLY `what is A plus 7?`."""
    rows = make_rung_examples("R1b8", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b8", n=2000, seed=42, split="held_out")
    for ex in rows:
        q = ex["question"]
        assert q.startswith("what is "), f"R1b8 prefix violated: {q!r}"
        assert q.endswith(" plus 7?"), f"R1b8 must end ' plus 7?': {q!r}"
        assert q.count(" plus ") == 1, f"R1b8 must contain ' plus ' exactly once: {q!r}"


def test_generator_r1b8_train_contains_all_one_digit() -> None:
    """R1b8 train MUST contain all 9 one_digit A values [1, 9]."""
    train = make_rung_examples("R1b8", n=4000, seed=42, split="train")
    train_as = {_r1b8_a(ex) for ex in train}
    one_digit_as = {a for a in train_as if 1 <= a <= 9}
    assert one_digit_as == set(range(1, 10)), (
        f"R1b8 train missing one_digit values: expected {{1..9}}, got {sorted(one_digit_as)}"
    )


def test_generator_r1b8_holdout_has_zero_one_digit() -> None:
    """R1b8 held_out MUST contain ZERO one_digit rows."""
    held = make_rung_examples("R1b8", n=4000, seed=42, split="held_out")
    held_as = {_r1b8_a(ex) for ex in held}
    one_digit_held = {a for a in held_as if 1 <= a <= 9}
    assert one_digit_held == set(), f"R1b8 held_out leaked one_digit rows: {sorted(one_digit_held)}"


def test_generator_r1b8_holdout_has_18_unique_two_digit() -> None:
    """R1b8 held_out unique support = 18 two_digit A values (12 carry + 6 non-carry)."""
    held = make_rung_examples("R1b8", n=4000, seed=42, split="held_out")
    held_as = {_r1b8_a(ex) for ex in held}
    assert len(held_as) == 18, f"R1b8 held_out unique A count must be 18; got {len(held_as)}: {sorted(held_as)}"
    for A in held_as:
        assert 10 <= A <= 92, f"R1b8 held_out A out of two_digit range: {A}"


def test_generator_r1b8_holdout_carry_stratified_12_plus_6() -> None:
    """R1b8 held_out: EXACTLY 12 carry + 6 non-carry two_digit values
    (carry-units {3..9} since K=7). Codex msg 1779550489408 spec."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b8
    _, held = _enumerate_partition_r1b8(seed=42)
    carry_held = sorted(a for a in held if (a % 10) in (3, 4, 5, 6, 7, 8, 9))
    non_carry_held = sorted(a for a in held if 10 <= a <= 92 and (a % 10) < 3)
    assert len(carry_held) == 12, f"R1b8 carry heldout must be 12; got {len(carry_held)}: {carry_held}"
    assert len(non_carry_held) == 6, f"R1b8 non-carry heldout must be 6; got {len(non_carry_held)}: {non_carry_held}"


def test_generator_r1b8_train_has_74_unique() -> None:
    """R1b8 train unique support = 74 A values (9 one_digit + 44 carry + 21 non-carry)."""
    train = make_rung_examples("R1b8", n=8000, seed=42, split="train")
    train_as = {_r1b8_a(ex) for ex in train}
    assert len(train_as) == 74, (
        f"R1b8 train unique A count must be 74 (9 one + 44 carry + 21 non-carry); got {len(train_as)}"
    )


def test_generator_r1b8_partition_carry_bucket_sizes() -> None:
    """`_enumerate_partition_r1b8` carry bucket: 56 vals -> 44 train + 12 held.
    Non-carry bucket: 27 vals -> 21 train + 6 held."""
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b8
    train, held = _enumerate_partition_r1b8(seed=42)
    carry_train = {a for a in train if 10 <= a <= 92 and (a % 10) in (3, 4, 5, 6, 7, 8, 9)}
    carry_held = {a for a in held if (a % 10) in (3, 4, 5, 6, 7, 8, 9)}
    non_carry_train = {a for a in train if 10 <= a <= 92 and (a % 10) < 3}
    non_carry_held = {a for a in held if 10 <= a <= 92 and (a % 10) < 3}
    assert len(carry_train) == 44, f"carry train: {len(carry_train)}"
    assert len(carry_held) == 12, f"carry held: {len(carry_held)}"
    assert len(non_carry_train) == 21, f"non-carry train: {len(non_carry_train)}"
    assert len(non_carry_held) == 6, f"non-carry held: {len(non_carry_held)}"
    assert (carry_train | carry_held | non_carry_train | non_carry_held) == set(range(10, 93))


def test_generator_r1b8_expected_matches_arithmetic() -> None:
    """expected == A + 7 for every R1b8 row."""
    rows = make_rung_examples("R1b8", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b8", n=2000, seed=42, split="held_out")
    for ex in rows:
        A = _r1b8_a(ex)
        assert ex["expected"] == A + 7, f"R1b8 arithmetic mismatch: A={A}, expected={ex['expected']}"


def test_generator_r1b8_output_in_8_to_99() -> None:
    """R1b8 outputs in [8, 99]."""
    rows = make_rung_examples("R1b8", n=2000, seed=42, split="train") + \
           make_rung_examples("R1b8", n=2000, seed=42, split="held_out")
    for ex in rows:
        assert 8 <= ex["expected"] <= 99, f"R1b8 output OOR: {ex}"


def test_generator_r1b8_rung_field() -> None:
    """Every R1b8 row carries rung='R1b8'."""
    rows = make_rung_examples("R1b8", n=200, seed=42, split="train") + \
           make_rung_examples("R1b8", n=200, seed=42, split="held_out")
    for ex in rows:
        assert ex["rung"] == "R1b8", f"R1b8 rung field violated: {ex!r}"


def test_r1b8_one_digit_audit_returns_9_rows() -> None:
    from calm.hrm_text_158.curriculum import r1b8_one_digit_audit_rows
    rows = r1b8_one_digit_audit_rows(seed=42)
    assert len(rows) == 9, f"audit must return 9 rows; got {len(rows)}"


def test_r1b8_one_digit_audit_seed_invariant() -> None:
    from calm.hrm_text_158.curriculum import r1b8_one_digit_audit_rows
    rows_a = r1b8_one_digit_audit_rows(seed=42)
    rows_b = r1b8_one_digit_audit_rows(seed=777)
    rows_c = r1b8_one_digit_audit_rows(seed=0)
    assert rows_a == rows_b == rows_c, "R1b8 audit must be seed-invariant"


def test_r1b8_one_digit_audit_covers_full_domain() -> None:
    """R1b8 audit covers A in [1, 9] exhaustively; expected = A + 7."""
    from calm.hrm_text_158.curriculum import r1b8_one_digit_audit_rows
    rows = r1b8_one_digit_audit_rows()
    audit_as = sorted(int(r["question"].split()[2]) for r in rows)
    assert audit_as == list(range(1, 10)), f"audit A coverage: {audit_as}"
    for r in rows:
        A = int(r["question"].split()[2])
        assert r["question"] == f"what is {A} plus 7?", f"audit question: {r!r}"
        assert r["expected"] == A + 7, f"audit expected: {r!r}"
        assert r["rung"] == "R1b8", f"audit rung field: {r!r}"


def test_r1b8_one_digit_audit_importable_from_package() -> None:
    from calm.hrm_text_158.curriculum import r1b8_one_digit_audit_rows
    assert callable(r1b8_one_digit_audit_rows)


def test_r1b8_audit_disjoint_from_holdout() -> None:
    from calm.hrm_text_158.curriculum import r1b8_one_digit_audit_rows
    audit_rows = r1b8_one_digit_audit_rows()
    held = make_rung_examples("R1b8", n=4000, seed=42, split="held_out")
    audit_qs = {r["question"] for r in audit_rows}
    held_qs = {ex["question"] for ex in held}
    overlap = audit_qs & held_qs
    assert not overlap, f"R1b8 audit ∩ heldout: {overlap}"


def test_r1b8_default_chain_contains_r1b8_no_overlap() -> None:
    """Default active chain (with R1b8) passes cross-rung invariant."""
    splits = build_rung_splits(
        n_train=2000, n_held_out=400, seed=42,
        rungs=("R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", "R1b6", "R1b7", "R1b8"),
    )
    assert "R1b8" in splits, f"R1b8 missing: {list(splits)}"
    assert_no_train_holdout_overlap(splits)


def test_r1b8_build_rung_splits_default_includes_r1b8() -> None:
    splits = build_rung_splits(n_train=200, n_held_out=80, seed=42)
    assert "R1b8" in splits, f"R1b8 must be in default chain; got {list(splits)}"


def test_r1b8_partition_one_digit_exhaustive() -> None:
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b8
    train, held = _enumerate_partition_r1b8(seed=42)
    one_digit_train = {a for a in train if 1 <= a <= 9}
    one_digit_held = {a for a in held if 1 <= a <= 9}
...
def test_r1b8_partition_train_held_disjoint() -> None:
    from calm.hrm_text_158.curriculum.generators import _enumerate_partition_r1b8
    train, held = _enumerate_partition_r1b8(seed=17)
    assert train.isdisjoint(held), f"R1b8 partition train ∩ held: {sorted(train & held)}"


def test_r1b8_partition_stable_across_pythonhashseed() -> None:
    ...
    out2 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "777"}).decode().strip()
    out3 = subprocess.check_output([sys.executable, "-c", snippet], env={**os.environ, "PYTHONHASHSEED": "random"}).decode().strip()
    assert out1 == out2 == out3, "R1b8 partition diverged across PYTHONHASHSEED"


def test_r1b8_in_one_digit_audit_registry() -> None:
    from scripts.probe_hrm_text_158 import ONE_DIGIT_AUDIT_REGISTRY
    assert "R1b8" in ONE_DIGIT_AUDIT_REGISTRY, f"R1b8 missing: {list(ONE_DIGIT_AUDIT_REGISTRY)}"
    rows = ONE_DIGIT_AUDIT_REGISTRY["R1b8"](seed=42)
    assert len(rows) == 9
    for r in rows:
        assert r["rung"] == "R1b8"


def test_r1b8_positional_priors_match_active_chain() -> None:
    """Trainer's `_resolve_prior_rungs("R1b8")` returns the active chain
    {R0, R1, R1b1, R1b2, R1b3, R1b4v2, R1b5, R1b6, R1b7}."""
    from calm.hrm_text_158.curriculum.replay import _resolve_prior_rungs
    priors = _resolve_prior_rungs("R1b8", replay_rungs_arg=None)
    assert priors == ["R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", "R1b6", "R1b7"], (
        f"R1b8 priors: {priors}"
    )
...
# ============================================================================ #
# Slice E.1 L0c generator tests (codex msg 1779571151811-d3f6bc4f +1 implement
...
def test_generator_l0c_NOT_in_diagnosis_only() -> None:
    """L0c is the active third language-axis rung; NOT diagnosis-only."""
    from calm.hrm_text_158.curriculum.replay import DIAGNOSIS_ONLY_RUNGS
    assert "L0c" not in DIAGNOSIS_ONLY_RUNGS, (
        f"L0c must NOT be diagnosis-only; got {sorted(DIAGNOSIS_ONLY_RUNGS)}"
    )


def test_generator_l0c_in_default_chain() -> None:
    """L0c must appear in `build_rung_splits(...)` default rungs after
    L0b (codex msg 1779571151811 active-chain inclusion)."""
    splits = build_rung_splits(n_train=200, n_held_out=80, seed=42)
    assert "L0c" in splits, f"L0c must be in default chain; got {list(splits)}"


def test_generator_l0c_train_holdout_exact_row_disjoint() -> None:
    """L0c train + held_out exact-row disjoint at n=2000 sampling."""
    train = make_rung_examples("L0c", n=2000, seed=42, split="train")
    held = make_rung_examples("L0c", n=2000, seed=42, split="held_out")
    train_set = {(ex["question"], ex["expected"]) for ex in train}
    held_set = {(ex["question"], ex["expected"]) for ex in held}
    overlap = train_set & held_set
    assert not overlap, f"L0c train/held share rows: {sorted(overlap)[:5]}"


def test_generator_l0c_template_equals_what_suffix() -> None:
    """L0c emits ONLY `<expr> equals what?` (interrogative-suffix form,
    distinct from L0a's `what's...?` AND L0b's `calculate...`)."""
    rows = make_rung_examples("L0c", n=2000, seed=42, split="train") + \
           make_rung_examples("L0c", n=500, seed=42, split="held_out")
    for ex in rows:
        q = ex["question"]
        assert q.endswith(" equals what?"), f"L0c row must end ` equals what?`: {q!r}"
        assert not q.startswith("what is "), f"L0c must NOT use canonical math `what is `: {q!r}"
        assert not q.startswith("what's "), f"L0c must NOT use L0a's `what's `: {q!r}"
        assert not q.startswith("calculate "), f"L0c must NOT use L0b's `calculate `: {q!r}"


def test_generator_l0c_rung_field() -> None:
    """Every L0c row carries rung='L0c'."""
    rows = make_rung_examples("L0c", n=500, seed=42, split="train") + \
           make_rung_examples("L0c", n=100, seed=42, split="held_out")
    for ex in rows:
        assert ex["rung"] == "L0c", f"L0c rung field violated: {ex!r}"


def test_generator_l0c_train_unique_count_184() -> None:
    """L0c train pool: 184 unique rows (identical to L0a/L0b partition size)."""
    train = make_rung_examples("L0c", n=20000, seed=42, split="train")
    unique_q = {ex["question"] for ex in train}
    assert len(unique_q) == 184, (
        f"L0c train unique_count must be 184; got {len(unique_q)}"
    )


def test_generator_l0c_held_unique_count_46() -> None:
    """L0c held pool: 46 unique rows."""
    held = make_rung_examples("L0c", n=5000, seed=42, split="held_out")
    unique_q = {ex["question"] for ex in held}
    assert len(unique_q) == 46, (
        f"L0c held unique_count must be 46; got {len(unique_q)}"
    )


def test_l0c_positional_priors_include_l0a_l0b_exclude_r1b10() -> None:
    """L0c positional priors = R0..R1b9 + L0a + L0b (13 priors). R1b10
    still filtered out by DIAGNOSIS_ONLY_RUNGS; L0a AND L0b stay IN so
    L0c training preserves BOTH prior paraphrase axes."""
    from calm.hrm_text_158.curriculum.replay import _resolve_prior_rungs
    priors = _resolve_prior_rungs("L0c", replay_rungs_arg=None)
    assert priors == ["R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2",
    ...
    assert "R1b10" not in priors
    assert "L0a" in priors
    assert "L0b" in priors


def test_l0a_l0b_l0c_question_sets_fully_disjoint() -> None:
    """L0a, L0b, L0c paraphrase pools must NOT share any question strings.
    `what's <expr>?` vs `calculate <expr>.` vs `<expr> equals what?` are
    distinct templates by construction; pin as cross-language-rung
    invariant across all three rungs."""
    l0a_train = make_rung_examples("L0a", n=20000, seed=42, split="train")
    l0b_train = make_rung_examples("L0b", n=20000, seed=42, split="train")
    l0c_train = make_rung_examples("L0c", n=20000, seed=42, split="train")
    l0a_set = {ex["question"] for ex in l0a_train}
    l0b_set = {ex["question"] for ex in l0b_train}
    l0c_set = {ex["question"] for ex in l0c_train}
    assert not (l0a_set & l0c_set), (
        f"L0a/L0c overlap: {sorted(l0a_set & l0c_set)[:5]}"
    )
    assert not (l0b_set & l0c_set), (
        f"L0b/L0c overlap: {sorted(l0b_set & l0c_set)[:5]}"
    )
...
def test_cross_rung_no_train_holdout_overlap() -> None:
    """Active-chain cross-rung invariant per codex msgs 1779488238721-49f03cc9
    (R1b4v2 advance at b368b81 via seed=2) + 1779523412979-ff88b885 (R1b5
    ...
    "R1b3", "R1b4v2", "R1b5", "R1b6", "R1b7", "R1b8", "R3", "R4", "R5",
    "R6")` (R1b8 added per codex msg 1779550489408-f40f66ab after R1b7
    commit 682659b + A0 exhaustive audit PASS). Diagnosis-only and OUT
    of default: R1b2a, R1b, R1b4, R2, R2a. The invariant asserts no row
    in any rung's held_out appears in any rung's train set across the
    active chain only."""
    splits = build_rung_splits(n_train=200, n_held_out=50, seed=42)
    assert_no_train_holdout_overlap(splits)
    # Active chain: R0, R1, R1b1, R1b2, R1b3, R1b4v2, R1b5, R1b6, R1b7, R1b8,
    # R1b9, L0a, L0b, L0c, R3-R6
    # (R1b2a/R1b/R1b4/R1b10/R2/R2a diagnosis-only; R7 = GSM8k)
    assert set(splits.keys()) == {"R0", "R1", "R1b1", "R1b2", "R1b3", "R1b4v2", "R1b5", "R1b6", "R1b7", "R1b8", "R1b9", "L0a", "L0b", "L0c", "R3", "R4", "R5", "R6"}
