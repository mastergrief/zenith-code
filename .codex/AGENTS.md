...
No subagents, no exception: work directly with Edit/Write/Read/Grep/Bash for the active authorized role, and perform every edit, validation run, and receipt yourself. ai-room collaboration remains two independent top-level sessions, not a subagent pattern.
...
Named Codex roles do specialized slice work under the co-leads + gates. `plan-dev` is the default always-on mutating lane for HRM and main-repo docs/config/tooling/scripts/tests/curriculum/probe work; it performs the plan/packet/implementation/final receipt itself, with no delegated executor. **`test-operator` is Claude-carried, not a spawnable worker role**: under the standing auto-research directive Gabe's gates are waived (including pushes and GPU runs) and Claude runs frozen packets directly — but peer gates are never waived, so gate1_audit gate-1 freeze → co_lead gate-2 → persisted `+1` still precede every implement, commit, push, and launch. **No peer is codex-backed** — all are Claude peers spawned by `ai_room_spawn_claude` on legacy `codex*` handles (`codex_co_lead` `sol=true` GPT, `plan-dev` on handle `codex` Opus with no subagents, `gate1_audit` `grok=true` grok, `claude` `grok=true` orchestrator; `advisor` is the interactive Fable session); "Codex role" above means worker role on a codex handle. Mutating work routes to `plan-dev`, not the read-only co-lead handle, unless an explicit named exception says otherwise.
...
- Material gates: see `.codex/rules/AI_ROOM_COLLAB.md` §"Provenance / Ingress-Owned". Ordinary `+1 commit` does not authorize push unless the gate text is `+1 commit+push`.
