...
Full spec: `.codex/rules/workflow.md`
...
**ai-room collab is not a subagent pattern.** When the user directs
claude+codex collaboration via ai-room MCP, it's two independent
top-level sessions exchanging structured messages — not subagents
inside one session. See "AI Room Collaboration" below. The
no-subagents rule is unaffected.
...
## Config `.codex/` Editing Directive

When editing `.codex/` configs (agents, CLAUDE.md, commands, rules etc):
...
Full spec in `.codex/rules/architecture.md` "Substrate Pattern" section.
