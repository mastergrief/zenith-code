# Distillation Pipeline — Historical receipts + operational reference

QLoRA fine-tuning / specialist distillation for `agents/distill/`.
Legacy/adjacent lane — consult when actively training traditional
base + specialist models, not on general HRM-158 sessions. Current
rules stub: `.codex/rules/distillation.md`.
...
- **4B reasoning base (Qwen)**: trained on Colab A100 (910
  ...
  fine-tuned Qwen 0/5** — correctness bugs (hallucinated Node.js
  `beforeOOM` API, broken Postgres `FOR UPDATE SKIP LOCKED` queue,
  regex-on-hostname SSRF check). See
  `.claude/MEMORY/evals/2026-04-07_qwen4b_vs_gemma4_e4b.md`.
...
## Training Data (`agents/distill/data/`, gitignored except hand-written)

- `claude_reasoning.jsonl` — 910 merged examples (re-filtered pass)
- `coding_reasoning_claude.jsonl` — 547 hand-written coding examples (committed)
- `claude_reasoning_filtered.jsonl` — 832 filtered HuggingFace (intermediate)
- `claude_reasoning_prefilter.jsonl` — backup of pre-filter merged data
- `orchestrator.jsonl` — 252 routing examples
- `orchestrator_claude.jsonl` — 121 Claude-authored routing (committed)
...
- Current rules (stub): `.codex/rules/distillation.md`
- Broader training: `.codex/rules/training.md`
- Auto-CALM data collection: `.codex/rules/calm.md`
